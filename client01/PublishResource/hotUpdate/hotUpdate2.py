            ...
            for root,folders,files in os.walk(fulldir):
                ...
                for sfile in files:
                    ...
                    if (os.path.getsize(zippath) >= zipsizelimit):
                        ...
                        zf = zipfile.ZipFile(zippath,mode='w',compression=zipfile.ZIP_DEFLATED)
                        ziplist.append(adir+'_'+str(fileidx)+'.zip')
                        fileidx = fileidx + 1
            zf.close()
    printdebug(ziplist)
...
def outputManifest():
    ...
    if not os.path.exists(outfile_name):
        ...
        for fullpath in filelist:
            # if (len(ziplist) > 0):
                # if not (os.path.basename(fullpath) in ziplist):
                    # continue
            edict ={}
        ...
        with open(outfile_name,'r') as file_object:
            jsonex = json.load(file_object)
            
        for edict in jsonex['assets']:
            if (len(ziplist) > 0):
                if (edict['name'] in ziplist):
                    fullpath = os.path.join(Res_ZipDone,edict['name'])
                    edict['size'] = round(os.path.getsize(fullpath)*0.001,3)
                    edict['md5'] =  getMd5(fullpath)
                    edict['crc'] = str(getCRC16(fullpath))
                    print('%s = %.3f bytes crc = %s md5 = %s '% (edict['name'], edict['size'],edict['crc'],edict['md5']))
...
def initPath():
    ...
    #exportlist =['audio','Lang','Spine','txt']
    if os.path.exists(assignDir):
        global exportlist
        with open(assignDir,'r')as file:
            lines = file.readlines()
        for line in lines:
            line = line.replace('\n','') #去掉換行
            exportlist.append(line)
            
    printdebug("exportlist :--------------------------------")
    printdebug(exportlist)
    printdebug("exportlist :--------------------------------")
