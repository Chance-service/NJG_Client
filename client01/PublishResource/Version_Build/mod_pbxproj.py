...
class PBXFileReference(PBXType):
    ...
    types = {
        ...
        '.cc': ('sourcecode.cpp.cpp', 'PBXSourcesBuildPhase'),
        ...
        '.ini': ('text', 'PBXResourcesBuildPhase'),
        '.dlt': ('text', 'PBXResourcesBuildPhase'),
        '.sha': ('text', 'PBXResourcesBuildPhase'),
        ...
        '.ccbi': ('file', 'PBXResourcesBuildPhase'),
        '.mp3': ('audio.mp3', 'PBXResourcesBuildPhase'),
        '.cfg': ('text', 'PBXResourcesBuildPhase'),
        '.php': ('text.script.php', 'PBXResourcesBuildPhase'),
        '.dll': ('file', None),
        '.exe': ('file', None),
        '.pdb': ('file', None),
    ...
    def guess_file_type(self, ignore_unknown_type=False):
        ...
        #print 'build_phase = %s' % build_phase
        #print 'f_type = ' + f_type
        ...
        if f_type == '?' and not ignore_unknown_type:
            #print self.get('path')
            #print os.path.isdir(self.get('path'))
...
class XcodeProject(PBXDict):
    ...
    def get_or_create_group(self, name, path=None, parent=None, tree='SOURCE_ROOT' ):
        if not name:
            return None
        ...
        grp = PBXGroup.Create(name, path, tree)
    ...
    def add_file(self, f_path, parent=None, tree='SOURCE_ROOT', create_build_files=True, weak=False, ignore_unknown_type=False):
        ...
        #print 'file_ref.lastKnownFileType = ' + file_ref.lastKnownFileType
    ...
    def remove_group(self, grp):
        grp_refs = [g for g in self.objects.values() if g.get('isa') == 'PBXGroup' and grp.has_child(g.id)]
        for grp_ref in grp_refs:
            self.remove_group(grp_ref)
            
        file_refs = [f for f in self.objects.values() if f.get('isa') == 'PBXFileReference' and grp.has_child(f.id)]
        phases = [p for p in self.objects.values() if p.get('isa') == 'PBXResourcesBuildPhase' or p.get('isa') == 'PBXSourcesBuildPhase']
        for file_ref in file_refs:
            build_files = self.get_build_files(file_ref.id)
            for build_file in build_files:
                #phases = self.get_build_phases(file_ref.build_phase)
                for phase in phases:
                    phase.remove_build_file(build_file.id)
                self.objects.remove(build_file.id)
            self.objects.remove(file_ref.id)
        self.objects.remove(grp.id)
        self.modified = True
    
    def remove_group_by_name(self, name):
        grps = self.get_groups_by_name(name)
        for grp in grps:
