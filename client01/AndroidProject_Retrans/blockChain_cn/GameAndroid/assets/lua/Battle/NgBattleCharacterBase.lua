...
function NgBattleCharacterBase:castSpecialBattlePassive(chaNode, fList, eList, skillId)
    ...
    if skillType ~= 0 then
        --if NewBattleUtil:castAutoSkill(skillId, chaNode) then
        --    local sceneHelper = require("Battle.NgFightSceneHelper")
        --    local skillGroupId = sceneHelper:getNewSkillGroupId()
        --    local resultTable = {}
        --    local acrionResultTable = {}
        --    local allTargetTable = {}
        --    LOG_UTIL:setPreLog(chaNode, resultTable)
        --    LOG_UTIL:addLog(CONST.LogActionType.CAST_SKILL, chaNode, nil, skillId, skillGroupId, {}, {})
        --
        --    resultTable = SkillManager:runPassive(skillId, chaNode, chaNode.target, resultTable, chaNode.skillData[skillType][skillId])
        --
        --    LOG_UTIL:setPreLog(chaNode, resultTable)
        --    CHAR_UTIL:calculateAllTable(chaNode, resultTable, nil, false, acrionResultTable, allTargetTable, skillId, {})
        --    LOG_UTIL:addLog(CONST.LogActionType.SKILL, chaNode, allTargetTable, skillId, skillGroupId, acrionResultTable, {})
        --end
    end
end
-------------------------------------------------------------
...
function NgBattleCharacterBase:beDot(chaNode, target, dmg, buffId, isSkipPre, isSkipAdd)
        ...
        for k, v in pairs(CONST.PASSIVE_TYPE_ID[CONST.PASSIVE_TRIGGER_TYPE.ENEMY_DEAD]) do -- 敵方死亡觸發被動
            for i = 1, #aliveIdTable do
                ...
                if NewBattleUtil:castPassiveSkill(list[aliveIdTable[i]], v, resultTable, allPassiveTable, CONST.PASSIVE_TRIGGER_TYPE.ENEMY_DEAD, { target }) then
                    local sceneHelper = require("Battle.NgFightSceneHelper")
                    local skillGroupId = sceneHelper:getNewSkillGroupId()
                    LOG_UTIL:addLog(CONST.LogActionType.CAST_SKILL, list[aliveIdTable[i]], nil, v, skillGroupId, actionResultTable, allPassiveTable)
                    LOG_UTIL:setPreLog(list[aliveIdTable[i]], resultTable)
                    CHAR_UTIL:calculateAllTable(list[i], resultTable, isSkipCal, actionResultTable, allTargetTable, v, allPassiveTable)   -- 全部傷害/治療/buff...處理
                    LOG_UTIL:addLog(CONST.LogActionType.SKILL, list[aliveIdTable[i]], allTargetTable, v, skillGroupId, actionResultTable, allPassiveTable)
                end
            end
        end
        local flist = NgBattleDataManager_getFriendList(target)
        ...
        for k, v in pairs(CONST.PASSIVE_TYPE_ID[CONST.PASSIVE_TRIGGER_TYPE.FRIEND_DEAD]) do -- 友方死亡觸發被動
            for i = 1, #aliveIdTable do
                ...
                if NewBattleUtil:castPassiveSkill(flist[aliveIdTable[i]], v, resultTable, allPassiveTable, CONST.PASSIVE_TRIGGER_TYPE.FRIEND_DEAD, { target }) then
                    local sceneHelper = require("Battle.NgFightSceneHelper")
                    local skillGroupId = sceneHelper:getNewSkillGroupId()
                    LOG_UTIL:addLog(CONST.LogActionType.CAST_SKILL, flist[aliveIdTable[i]], nil, v, skillGroupId, actionResultTable, allPassiveTable)
                    LOG_UTIL:setPreLog(flist[aliveIdTable[i]], resultTable)
                    CHAR_UTIL:calculateAllTable(list[i], resultTable, isSkipCal, actionResultTable, allTargetTable, v, allPassiveTable)   -- 全部傷害/治療/buff...處理
                    LOG_UTIL:addLog(CONST.LogActionType.SKILL, flist[aliveIdTable[i]], allTargetTable, v, skillGroupId, actionResultTable, allPassiveTable)
                end
            end
        end
        for k, v in pairs(CONST.PASSIVE_TYPE_ID[CONST.PASSIVE_TRIGGER_TYPE.KILL_ENEMY]) do -- 自身擊殺敵方觸發被動
    ...
    if not isSkipAdd then
        ...
        local skillGroupId = sceneHelper:getNewSkillGroupId()
        LOG_UTIL:addLog(CONST.LogActionType.BUFF, target, { target }, buffId, skillGroupId, { CONST.LogActionResultType.HIT }, { })
    end
    if buffId then
...
function NgBattleCharacterBase:setOnAnimationFunction(chaNode)
    chaNode.onPlaySpineAnimation = function()
        ...
        if playAniName == CONST.ANI_ACT.ATTACK then   -- 普通攻擊
            LOG_UTIL:addLog(CONST.LogActionType.CAST_ATTACK, chaNode, nil, nil, skillGroupId, {}, {}, chaNode.ATTACK_PARAMS["play_spine_time"])
            ...
            for k, v in pairs(CONST.PASSIVE_TYPE_ID[CONST.PASSIVE_TRIGGER_TYPE.ENEMY_CAST_ACTIVE_SKILL]) do -- 敵方施放主動技能時觸發
                for i = 1, #aliveIdTable do
                    ...
                    if NewBattleUtil:castPassiveSkill(elist[aliveIdTable[i]], v, resultTable, allPassiveTable, CONST.PASSIVE_TRIGGER_TYPE.ENEMY_CAST_ACTIVE_SKILL, { chaNode }) then
                        local sceneHelper = require("Battle.NgFightSceneHelper")
                        local skillGroupId = sceneHelper:getNewSkillGroupId()
                        LOG_UTIL:addLog(CONST.LogActionType.CAST_SKILL, elist[aliveIdTable[i]], nil, v, skillGroupId, actionResultTable, allPassiveTable)
                        LOG_UTIL:setPreLog(elist[aliveIdTable[i]], resultTable)
                        CHAR_UTIL:calculateAllTable(elist[i], resultTable, isSkipCal, actionResultTable, allTargetTable, v, allPassiveTable)   -- 全部傷害/治療/buff...處理
                        LOG_UTIL:addLog(CONST.LogActionType.SKILL, elist[aliveIdTable[i]], allTargetTable, v, skillGroupId, actionResultTable, allPassiveTable)
                    end
                end
            end

            LOG_UTIL:addLog(CONST.LogActionType.CAST_SKILL, chaNode, nil, skillId, skillGroupId, allTargetTable, allPassiveTable, chaNode.ATTACK_PARAMS["play_spine_time"])
            LOG_UTIL:addAttackLog(chaNode, skillId, skillGroupId)
...
function NgBattleCharacterBase:onFunction(tag, eventName)
    ...
    if string.find(eventName, "_hit") then  
        if chaNode.idx == 2 then
            local a = 1
        end
        local buffEvent = CONST.ADD_BUFF_COUNT_EVENT.CAST_ATTACK
        local triggerBuffList = BuffManager:specialBuffEffect(chaNode.buffData, buffEvent, chaNode, chaNode.target, skillId)    -- 觸發buff效果
        -- 特殊buff觸發處理
	...
	elseif eventName == "hit" then      
        ...
        if resultTable then
            ...
            LOG_UTIL:addLog(logActionType, chaNode, allTargetTable, attackLog.skillId, attackLog.skillGroupId, actionResultTable, allPassiveTable)
        end
        -- 檢查被動技能發動(需等log記錄完)
