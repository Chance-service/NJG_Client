...
function Inst:onHandNode()
    ...
    elseif Jump_type == 1 then
        -- 回憶武器改跳滿等預覽介面
        local itemId = self.subPageCfg.data.Jump
        local isAw = InfoAccesser:getIsAncientWeaponSoul("30000_" .. itemId .. "_1")
        if isAw then
            local itemCfg = { type = Const_pb.EQUIP * 10000, itemId = itemId }
            GameUtil:showTip(nil, itemCfg, nil)
        else
