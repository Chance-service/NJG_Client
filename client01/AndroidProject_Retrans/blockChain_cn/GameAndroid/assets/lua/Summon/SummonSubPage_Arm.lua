...
local thisPageName = "SummonSubPage_Arm"
...
--[[ 協定 ]]
Inst.opcodes["ACTIVITY178_CALL_OF_EQUIP_INFO_S"] = HP_pb.ACTIVITY178_CALL_OF_EQUIP_INFO_S
Inst.opcodes["ACTIVITY178_CALL_OF_EQUIP_DRAW_S"] = HP_pb.ACTIVITY178_CALL_OF_EQUIP_DRAW_S
...
function Inst:onReceivePacket(packet)
    ...
    if self:handleSummonError(packet, HP_pb.ACTIVITY178_CALL_OF_EQUIP_DRAW_C) == true then return end

    if packet.opcode == HP_pb.ACTIVITY178_CALL_OF_EQUIP_INFO_S or
       packet.opcode == HP_pb.ACTIVITY178_CALL_OF_EQUIP_DRAW_S then
        if packet.opcode == HP_pb.ACTIVITY178_CALL_OF_EQUIP_INFO_S then
            print("ACTIVITY178_CALL_OF_EQUIP_INFO_S")
        end
...
function Inst:onEnter_content ()
    ...
    if self.subPageCfg.spineBG ~= nil then
        ...
        self.spineBG:registerFunctionHandler("COMPLETE", function(unknownArg, tag, eventName)
            if eventName == "COMPLETE" or eventName == "End" then
...
function Inst:loadSummonSpine ()
    ...
    if self.spineSummon == nil and self.subPageCfg.spineSummon ~= nil then
        ...
        self.spineSummon:registerFunctionHandler("COMPLETE", function(unknownArg, tag, eventName)
            if eventName == "COMPLETE" or eventName == "End" then
        ...
        self.spineSummon:runAnimation(1, self.subPageCfg.spineAnimName_summon_idle, 0)
...
function Inst:sendRequestInfo (isShowLoading)
    ...
    if IS_MOCK then
        self:onReceivePacket({
            opcode = HP_pb.ACTIVITY178_CALL_OF_EQUIP_INFO_S,
            msg = {
                freeTimes = 0,
                onceCostGold = 120,
                tenCostGold = 1200,
    ...
    common:sendEmptyPacket(HP_pb.ACTIVITY178_CALL_OF_EQUIP_INFO_C, isShowLoading)
end
...
function Inst:sendSummon1 ()
    if IS_MOCK then
        self:onReceivePacket({
            opcode = HP_pb.ACTIVITY178_CALL_OF_EQUIP_DRAW_S,
            msg = {
                reward = "10000_1001_1",
            }
        })
    ...
    self.task = ALFManager:loadSpineTask(spineFolderAndName[1] .. "/", spineFolderAndName[2], texNum, function() 
       if not Inst.JumpAnim then
        ...
        msg.times = 1
        common:sendPacket(HP_pb.ACTIVITY178_CALL_OF_EQUIP_DRAW_C, msg, true)
...
function SummonPageArm_calIsShowRedPoint(msg)
    if not msg then
        return false
    end
