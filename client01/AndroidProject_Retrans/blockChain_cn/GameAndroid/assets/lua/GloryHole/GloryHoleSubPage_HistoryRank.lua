...
function GloryHoleRankBase:onDailyReward(container)
    ...
    NodeHelper:setStringForLabel(container,{mDailyRankTitle=common:getLanguageString("@GloryHoleranking03")})
end
...
function GloryHoleRankBase:BuildRewardScrollview(container)
    ...
    for i=1,#RewardCfg do
        local Data=RewardCfg[i].BestReward
...
function GloryHoleRankBase:onEnter(container)
    ...
    StringTable["mDailyTitle"]=common:getLanguageString("@GloryHoleranking03")
...
function GloryHoleHistoryRank_refresh()
...
function GloryHoleRankBase:refresh(container)
    ...
    if SelfTable.maxRank==2 then
        SpriteImg=GameConfig.ArenaRankingIcon[2]
    elseif SelfTable.maxRank==3 then
    ...
    VisableTable["mSelfRankSprite"]=(SelfTable.maxRank<4 and SelfTable.maxRank~=0)
    StringTable["mSelfRankText"]=SelfTable.maxRank
    if SelfTable.maxRank==0 then
        StringTable["mSelfRankText"]="-"
    end
    StringTable["mPlayerScore"]=SelfTable.maxScore
...
function GloryHoleRankBase:initScrollView(container)
    local PageTable=GloryRankInfo.HistoryRank
