...
local ScorllBtnMap={
    ...
    --{key="Star",normal="Lobby_Btn_9.png",press="Lobby_Btn_9.png",fun="onStar",Pos="Left",Lock=GameConfig.LOCK_PAGE_KEY.WISHING_WELL,isRed="WishingWell.WishingWellPage",idx=7},
    --RightScrollview
    ...
    {key="Reward",normal="Lobby_Btn_7.png",press="Lobby_Btn_7.png",fun="onReward",Pos="Right",Lock=GameConfig.LOCK_PAGE_KEY.SUMMON_900,isRed=RedPointManager.PAGE_IDS.LOBBY_PACKAGE_BTN,idx=12},
}
...
function MainScenePageInfo.onEnter(container)
    ...
    --191資料要求
     if ActivityInfo:getActivityIsOpenById(191) then
        NodeHelper:setNodesVisible(container,{mActivityNode = true})
    ...
    --    if (GuideManager.currGuideType == GuideManager.guideType.NEWBIE_GUIDE) and
    --       (GuideManager.currGuide[GuideManager.guideType.NEWBIE_GUIDE] < 10500) then   --~前導影片3的步驟
...
function MainScenePageInfo:handleScrollview(scrollview, pos, isShort,showTable)
    ...
    if isShort then
        ...
        if pos == "Left" then 
            ...
            cell=self:CreateBtn(ScorllBtnMap[7])
        end
...
function MainScenePageInfo:GetGloryTime()
     --GloryHole
    local bannerCfgs = ConfigManager:getBannerCfg()
    for key,data in pairs (bannerCfgs) do
        if data.Page == 175 then
...
function BtnItem:onRefreshContent(ccRoot)
    ...
    VisibleMap["mTimeNode"]= self.Time>0 
...
function setPayUrl()
    ...
    if Golb_Platform_Info.is_h365 and Golb_Platform_Info.is_Android then
        if (NodeHelper:isDebug()) then
            ...
            elseif (Svrid == 2) then -- 正式服2
	    		PayUrl = "https://gs02.paycallback.quantagalaxies.com/payNotice?params="
    ...
    if Golb_Platform_Info.is_kuso and Golb_Platform_Info.is_Android then
        if (NodeHelper:isDebug()) then
            if (Svrid == 1) then    --正式服
                PayUrl = "https://paycallback.quantagalaxies.com/KusoPay?params="
            elseif (Svrid == 2) then -- 正式服2
