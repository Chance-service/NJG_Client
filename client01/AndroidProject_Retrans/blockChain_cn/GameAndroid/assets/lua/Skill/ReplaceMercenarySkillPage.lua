...
function SkillItem.onRefreshItemView(container)
	...
	--local newStr =GameMaths:stringAutoReturnForLua(skill.describe, common:getSkillDetailLength(), 0)
	
	local lb2Str = {
		mSkillName 		= skill.name,
	--	mSkillTex 		= newStr
	}
	NodeHelper:setCCHTMLLabel(container,"mSkillTex",CCSize(630,96),skill.describe,true)
...
function ReplaceSkillPageBase:getMercenaryInfo( container )
	local mercenaryStr = ServerDateManager:getInstance():getRoleInfoByIdForLua(thisRoleId);
