...
public class PlatformAPI extends SDKService {
	...
	private static String appId = "APPDZnofBIY5b64jHC";//"com.kusoplay.sample";
	private static String subId = "NG24";
	public static String aplusToken = "";
	private static boolean dologout = false;

	@Override
	public void onCreateSDK(){
		// 設定 KUSO SDK 遊戲資訊.
		if (CpsConst.cps == E_APLUS_CPS.CPS1) {
			subId = "NG24C1";
		}
	...
	public  void initTabDB()
	{
		...
		{
			if (CpsConst.cps == E_APLUS_CPS.CPS1) {
				tabDBId = "dnjkmvjagmkqnkdz";
			}
			else {
				tabDBId = "dnjkmvjagmkqnkdz"; //ojd31nr7523qt1pq
			}
			channel = "KUSO";
		}
		super.initTabDB();
	...
	public void toPay(PayInfo pay_info) throws JSONException{
		...
		obj.put("platform", "android_aplus");
		obj.put("payMoney", Double.toString(pay_info.price)); // usd
		obj.put("goodsId", pay_info.name);
		obj.put("goodsCount", pay_info.count);
		obj.put("serverId", Svrid);
		obj.put("test", "false");
		String callbackurl = payurl + escape(obj.toString()) + "&user=aplus";
		showlog("callbackurl: " + callbackurl);
		//金額單位:分 (1元=100分)
		showlog("pay order_serial: " + pay_info.order_serial);
		PlayCenter.shared.payOrder((int)pay_info.price * 100, Currency.CNY, GatewayCode.ALL, callbackurl, pay_info.description,
		...
		if (isCreate) {
			...
			PlayCenter.shared.login(new PlayCenterLoginListener() {
				...
				public void onLogin(boolean success, String id, String token,PlayCenterError error) {
					if (success){
						uuid = id;
						showlog("login  APLUS uuid: " + uuid);
						aplusToken = token;
						showlog("login  APLUS token: " + aplusToken);
					}
				}
			});
