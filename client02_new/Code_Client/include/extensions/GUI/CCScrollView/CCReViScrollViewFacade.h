...
#define Macro_FixedItemViewsMultiply 3
	...
	public:
		...
		*/
		...
		//获取到第一个child的ContentSize by zhenhui 2014/11/21
		cocos2d::CCSize getFirstChildContentSize();
		//根据offset,设置并刷新items by zhenhui 2014/11/21
		void setAndRefreshItemsByOffset(CCPoint offset);
		//根据index,设置并刷新items by zhenhui 2014/11/21
		void setAndRefreshItemsByIndex(int index);
		//设置是否针对每个item做 Bounced 操作。 类似于 IOS DATE PICKER的功能，在滑动结束后，回退到最近的index的位置，保证最上方的item显示完整。  by zhenhui 2014/11/21
		void setBouncedFlag(bool flag){
			m_bouncedFlag = flag;
		}
		bool getBouncedFlag(){
			return m_bouncedFlag;
		}
	//
	private:
	...
	private:
		...
		*/
		...
		//use for the bounced scrollview by zhenhui 2014/11/21
		//为scrollView 添加类似于 IOS DATE PICKER的功能，在滑动结束后，回退到最近的index的位置，保证最上方的item显示完整。
		bool m_bouncedFlag;
	};
