...
#define  LOG_TAG    "libOSHelpJni.cpp"
...
std::string getCurrentCountryJNI() {
	std::string country = (std::string)callPlatformSendMessageG2PJNI("getCurrentCountry", Json::Value(Json::objectValue).toStyledString());
	LOGD("getCurrentCountryJNI, retString=%s", country.c_str());
	return country;
}
