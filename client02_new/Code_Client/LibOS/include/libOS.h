...
struct GAME_CONFIG_STU
{
    ...
    std::string channel;     //渠�???
    std::string mta_key;     //MTA统计?�数
    bool        mtaEnable;   //?�否?�用MTA统计
    bool        flurryEnable;//?�否?�用Flurry统计
...
public:
	...
	void showInputbox(bool multiline,int InputMode, std::string content = "", bool chatState = false);//?��??��?fhr
...
//    void showInputbox(bool multiline, std::string content = "");
    ...
    void openURLHttps(const std::string& url);//appstore 大�???lvpeizong
...
private:
    ...
    GAME_CONFIG_STU m_gameconfig;//iOS?��??��??��?
public:
