...
public:
	...
	std::string getUserBasicInfoForLua() { 
		if (mUserPlayerInfo.IsInitialized() && mUserPlayerInfo.playerid() != 0)
		{ 
			return mUserPlayerInfo.SerializeAsString(); 
		}
		return "";
	};
	...
	std::string getUserRoleInfoForLua() 
	{
		if (mUserRoleInfo.IsInitialized())
		{
			return mUserRoleInfo.SerializeAsString();
		}
		return "";
	};
