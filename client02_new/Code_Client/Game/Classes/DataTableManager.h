...
public:
	...
	std::string     music;
	unsigned int    mainType;			//冒险分类，具体参考Adventure_MainType的定义, 
...
private:
	void readline(std::stringstream& _stream)
	{
		_stream
			...
			>>describe
			>>iconPic;
		name = convertToDict(name);
		describe = convertToDict(describe);
	}
...
private:
	...
	friend class LevelItemTableManager;
};
class LevelItemTableManager
	: public TableReader
	,public Singleton<LevelItemTableManager>
{
public:

	LevelItemTableManager()
	{
		init("roleLevelExp.txt");
	}

	typedef std::map<unsigned int, LevelItem* > LevelItemList;

	void init(const std::string& filename);
	const LevelItem * getLevelItemByID(unsigned int id);
private:
	virtual void readline(std::stringstream& _stream);
	LevelItemList mLevelItemList;
};
...
public:
	...
	unsigned int    FBshareSwitch;//FB分享开关
	std::string		FBUrl;//Facebook分享地址
	std::string		FaqUrl;//faq分享地址
	unsigned int		nSingnatureLenLimit;//签名个数限制
	std::string		FbInviteUrl;//fb邀请导向地址
	std::string		FbInvitePic;//fb邀请图片地址
...
private:
	void readline(std::stringstream& _stream)
	{
		_stream
			...
			>>FbInviteUrl
			>> FbInvitePic;
		loadingFrameSeversConnection = convertToDict(loadingFrameSeversConnection);
		loadinScenceMsg = convertToDict(loadinScenceMsg);
	}
...
private:
	void readline(std::stringstream& _stream)
	{
		_stream
			>>msgId
			>>msgContent;
		msgContent = convertToDict(msgContent);
	}
...
class ResourceConfigManager
	: public TableReader
	,public Singleton<ResourceConfigManager>
{
public:
	ResourceConfigManager()
	{
		init("ResourceConfig.ini");
	}
	typedef std::map<unsigned int, ResourceConfigItem* > ResourceConfigList;
	typedef ConstMapIterator<ResourceConfigList> ResourceConfigListIterator;	

	void init(const std::string& filename);	
	ResourceConfigItem* getResourceItem(const std::string& platform);
private:
	virtual void readline(std::stringstream& _stream);

	ResourceConfigList mResourceConfigList;
};


/////======================================
//多语言I18n和资源路径相关配置
class I18nLanguageCfgItem
{
public:
	int id;
	int languageType;
	std::string srcPath;
	int isTextLeft2Right;
	int isForce;

	I18nLanguageCfgItem() :isForce(0){}
...
class I18nLanguageCfgManager
	: public TableReader
	, public Singleton<I18nLanguageCfgManager>
{
public:
	I18nLanguageCfgManager()
	{
		init("txt/I18nConfig.txt");
	}
	typedef std::map<unsigned int, I18nLanguageCfgItem* > I18nConfigList;
	typedef ConstMapIterator<I18nConfigList> I18nConfigListIterator;

	void init(const std::string& filename);
	I18nLanguageCfgItem* getI18nItem(ccLanguageType languageType);
private:
	virtual void readline(std::stringstream& _stream);

	I18nConfigList mI18nConfigList;
};
...
/////======================================
//多语言I18n平台和语言列表相关的配置
class I18nPlatformCfgItem
{
public:
	int id;
	std::string platformName;
	//语言类型列表
	std::vector<unsigned int> languageTypeList;
	void readline(std::stringstream& _stream);
};

class I18nPlatformCfgManager
	: public TableReader
	, public Singleton<I18nPlatformCfgManager>
{
public:
	I18nPlatformCfgManager()
	{
		init("txt/I18nPlatform.cfg");
	}
	typedef std::map<unsigned int, I18nPlatformCfgItem* > I18nPlatformCfgList;
	typedef ConstMapIterator<I18nPlatformCfgList> I18nPlatformCfgListIterator;

	void init(const std::string& filename);
	I18nPlatformCfgItem* getI18nPlatformItem(const std::string& platform);
private:
	virtual void readline(std::stringstream& _stream);

	I18nPlatformCfgList mI18nPlatformCfgList;
};
...
public:
	int id;
	std::string tip;
	std::vector<unsigned int> tipList;
	void readline(std::stringstream& _stream);
};

class TipsManager
	: public TableReader
	, public Singleton<TipsManager>
{
public:
	TipsManager()
	{
	...
	typedef std::map<unsigned int, TipsItem* > TipsItemList;
	typedef ConstMapIterator<TipsItemList> TipsItemListIterator;

	void init(const std::string& filename);
	TipsItemListIterator getTipsIterator(void) 
	{
		return TipsItemListIterator(mTipsItemList.begin(), mTipsItemList.end());
	}
private:
	virtual void readline(std::stringstream& _stream);

	TipsItemList mTipsItemList;
};