...
{
	size_t hlpos = m_richText.find("<HL>");
	size_t hmpos = m_richText.find("<HM>");
	size_t hrpos = m_richText.find("<HR>");
	if ((hlpos != std::string::npos) | (hmpos != std::string::npos) | (hrpos != std::string::npos))
	{
		size_t tmpPos = (hlpos != std::string::npos) ? hlpos : (hmpos != std::string::npos) ? hmpos : hrpos;
		strategy[0] = (hlpos != std::string::npos) ? 0 : (hmpos != std::string::npos) ? 1 : 2;
		m_richText = m_richText.substr(tmpPos + std::strlen("<HL>"));
		size_t vbpos = m_richText.find("<VB>");
		size_t vmpos = m_richText.find("<VM>");
		size_t vtpos = m_richText.find("<VT>");
		if ((vbpos != std::string::npos) | (vmpos != std::string::npos) | (vtpos != std::string::npos))
		{
			size_t tmpPos = (vbpos != std::string::npos) ? vbpos : (vmpos != std::string::npos) ? vmpos : vtpos;
...
{
	size_t pos = m_richText.find("<VS=");
	if (pos != std::string::npos)
	{
		m_richText = m_richText.substr(pos + std::strlen("<VS="));
		size_t endPos = m_richText.find(">");
...
for (std::vector<CCTagedRichTTFContent*>::iterator inner_it = sameLineTTFs.begin(); inner_it != sameLineTTFs.end(); ++inner_it)\
{\
	CCTagedRichTTFContent* tmpContent = *inner_it;\
...
void CCTagedRichTTFContainer::buildRichText(const std::string& richText, int verSpace)
{
	...
	for (size_t pos = tmpText.find("</>"); pos != std::string::npos; pos = tmpText.find("</>"))
	{
		size_t endPos = pos + std::strlen("</>");
	...
	for(std::vector<std::string>::iterator it = subRichText.begin(); it != subRichText.end(); ++it)
	{
		CCTagedRichTTFContent* content = CCTagedRichTTFContent::create();
	...
	for (std::vector<CCTagedRichTTFContent*>::iterator it = richTTFs.begin(); it != richTTFs.end(); ++it)
	{
		CCTagedRichTTFContent* content = *it;
...
void CCTagedRichTTFContent::parseKeyValues(const std::string& text)
{
	...
	for (int i = 0; i <7; ++i)
	{
		size_t pos = valueStr.find(keys[i]);
		if (pos!= std::string::npos)
		{
	...
	size_t showStrEndpos = tmpShowStr.find("<");
...
{
	size_t headPos = valueStr.find("'");
	std::string tmpStr2 = valueStr.substr(headPos+1);
	size_t endPos = tmpStr2.find("'");
...
void CCTagedRichTTFContent::parseSuperLinkInfo(const std::string& text)
{
	...
	if (pos != std::string::npos)
	{
		std::string tmpStr = text.substr(0, pos);
