...
public: 
		...
		virtual void onReceivePacket(const int opcode, const ::google::protobuf::Message* packet)
		{
			...
			if (loginRet)
			{
				...
				std::string gPuidKey = "LoginGuestID";
				...
				if (sLoginName.length()>0)
				{
					...
					if (sLoginName.find("guest_") != std::string::npos)
					{
						cocos2d::CCUserDefault::sharedUserDefault()->setStringForKey(gPuidKey.c_str(), sLoginName);
					}
					std::string passkey = "PassKey";
					std::string aPwd = GamePrecedure::Get()->getDefaultPwd();
					cocos2d::CCUserDefault::sharedUserDefault()->setStringForKey(passkey.c_str(), aPwd);
					LoadingFrame* pLoginFrame = GamePrecedure::Get()->getLoadingFrame();
					if (pLoginFrame)
					{
						pLoginFrame->updateLocalSeverId();
						pLoginFrame->hideLoginUser();
					}
					else
					{
						CCLOG("LoadingFrame not get");
					}
				}
...
public: 
	...
	virtual void onReceivePacket(const int opcode, const ::google::protobuf::Message* packet)
	{
		...
		if(ret)
		{
			...
			GamePrecedure::Get()->setServerData(ret->data());
			GamePrecedure::Get()->setGotHeartBeatTime();
...
DEFINE_CREATE_REGISTER_HANDLER(OPErrorMsgHandle, ERROR_CODE)
{
	...
	//if (errorMsg->errcode() == 33001) return;
	...
	if (errorMsg->has_errmsg())
	{
		...
		if (errorMsg->has_errflag())
		{
			...
			if (errorFlag == 1)
			{
				...
				MessageHintPage::Msg_Hint(errorMsg->errmsg());
			}
		}else{
	...
	{
		//错误码机制，需要已登录就加载ErrMsgTableManager 的配置，并且判断hpCode，如果为登录回包，则需要用别的弹窗弹出。
		...
		if (errorMsg->has_errflag())
		{
			...
			if (errorFlag == 1)
			{
				...
				if (errMsgItem)
				{
					MessageHintPage::Msg_Hint(errMsgItem->msgContent);
				}
				else
					//@ERRORCODE_999999
					MessageHintPage::Msg_Hint(Language::Get()->getString("@ERRORCODE_999999").c_str());
				;

				//MessageHintPage::Msg_Hint(_errMsg);
			}
		}else{
			MSG_BOX(_errMsg);
		}
