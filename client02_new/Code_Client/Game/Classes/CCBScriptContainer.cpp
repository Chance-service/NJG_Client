...
CCBScriptContainer* CCBScriptContainer::create( const std::string& filename , int tag)
{
	...
	{
		//CCLog("willCCBScriptContainer::createFromNew1	%s %d %d", filename.c_str(), ccbsPool.size(),tag);
...
void CCBScriptContainer::onCloseKeyboard()
{
	RUN_SCRIPT_FUN("luaonCloseKeyboard");
}
void CCBScriptContainer::onOpenKeyboard()
{
	RUN_SCRIPT_FUN("luaonOpenKeyboard");
}
void CCBScriptContainer::OnKeyboardHightChange(int nHight)
{
	mKeyboardHight = nHight;
	RUN_SCRIPT_FUN("luaOnKeyboardHightChange");
}
...
void CCBScriptContainer::onFBShareBackMessage(bool success)
{
	mFbshareState = success;
	RUN_SCRIPT_FUN("luaFBShareBack");
}

void CCBScriptContainer::setDynamicIconTex(std::string frameName ,std::string iconName , std::string iconStr)
{
	//┨ⅷ떠쯹퇽쯑ㅸ챞쩳쩑⑾⑥CCBContainerㅊ
