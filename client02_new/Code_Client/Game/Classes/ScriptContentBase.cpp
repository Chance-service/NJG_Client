...
ScriptContentBase::~ScriptContentBase()
{
	if(mIsHttpImgListener)
	{
		HttpImg::getInstance()->removeListener(this);
	}
...
void ScriptContentBase::onHttpImgCompleted(std::string& imgName)
{
	mHttpImgName = imgName;
	RUN_SCRIPT_FUN("luaHttpImgCompleted");
}
...
void ScriptContentBase::addToHttpImgListener( const std::string& imgName )
{
	mIsHttpImgListener= true;
	HttpImg::getInstance()->addListener(this,imgName);
}
