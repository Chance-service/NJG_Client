...
MESSAGE_FACTORY_BEGIN(MsgRefreshRedPoint)
char pagename[128];
sscanf(paras.c_str(), "%128[^\t]", pagename);
ret->pageName = pagename;
MESSAGE_FACTORY_END(MsgRefreshRedPoint, MSG_REFRESH_REDPOINT);

MESSAGE_FACTORY_BEGIN(MsgSendPackageFailed)
int opcode;
sscanf(paras.c_str(), "%d", opcode);
ret->opcode = opcode;
MESSAGE_FACTORY_END(MsgSendPackageFailed, MSG_SEND_PACKAGE_FAILED);