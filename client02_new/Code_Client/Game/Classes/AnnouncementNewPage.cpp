...
void AnnouncementNewContent::refreshContent( void )
{
	...
	for (int i=0;i<vecLines.size();i++)
	{
		...
		if (sSeg.length() ==0) 
		{
			...
			//目前每行最多顯示27個字，配置在Varible.txt中的AnnouncementPageDescribe=27
			std::string des;
			int lines;
...
void AnnouncementNewPage::buildItem( void )
{
	...
	// 因為平台差異，如果當前item是本平台排除掉的訊息，那麼跳過這個item（例如多酷跳過微信）
	...
	while(it.hasMoreElements())
	{
		...
		{
			// 如果在excludedIds列表中找到此itemId，那麼過濾掉這個id
			continue;
		}
		// end
