...
void GraySprite::AddColorGrayToNode(CCNode * node)
{
	if (node){
		node->setShaderProgram(CCShaderCache::sharedShaderCache()->programForKey("ShaderPositionTextureGray"));
	}
}

void GraySprite::RemoveColorGrayToNode(CCNode * node)
{
	if (node){
		node->setShaderProgram(CCShaderCache::sharedShaderCache()->programForKey("ShaderPositionTextureColor"));
	}
}


void GraySprite::AddColorGray(CCSprite * spr)
{ 
	if (spr){

		spr->setShaderProgram(CCShaderCache::sharedShaderCache()->programForKey("ShaderPositionTextureGray"));

	}

}
void GraySprite::RemoveColorGray(CCSprite * spr)
{ 
	if (spr){
		spr->setShaderProgram(CCShaderCache::sharedShaderCache()->programForKey("ShaderPositionTextureColor"));
	}
}
