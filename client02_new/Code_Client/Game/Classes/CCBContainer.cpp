...
bool CCBContainer::onAssignCCBMemberVariable(cocos2d::CCObject* pTarget, const char* pMemberVariableName, cocos2d::CCNode* pNode)
{
	...
	//CCBI文件自身保證不重名節點
	std::transform(var.begin(), var.end(), var.begin(), tolower);//xinzheng 2013-05-27 重做的一版資源 存在命名大小寫出入，程式處理以支援
...
void CCBContainer::_menuItemClicked(cocos2d::CCObject * pSender)
{
	...
	{
		//提示所等待的opcode。但某些特定情況，如點擊-好友-邀請按鈕，會同時觸發onclickBtn  以及onContent，所以會出現“假等待”情況，可以忽略該情況
#ifdef _DEBUG
#ifdef WIN32
	...
	if (it != mMenus.end())
	{
		...
		if (CCScriptEngineManager::sharedManager()->getScriptEngine())
		{
			...
			CCLOG("Press button ===============================fun:%s", funname.c_str());
