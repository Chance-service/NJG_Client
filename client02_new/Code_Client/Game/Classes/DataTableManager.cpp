...
const std::string& convertToDict(const std::string& orignStr)
{
	std::string::size_type pos =  orignStr.find_first_of("@");
	if (pos != std::string::npos && pos == 0)
	{
		return Language::getInstance()->getString(orignStr);
	}
	return orignStr;
}
//-----------------------------------------------------------------------------------------------
...
//RoleExpTableManager
//-----------------------------------------------------------------------------------------------
void LevelItemTableManager::init( const std::string& filename )
{
	LevelItemList::iterator itr = mLevelItemList.begin();
	while(itr != mLevelItemList.end())
	{
		delete itr->second;
		itr->second =NULL;
		++itr;
	}
	mLevelItemList.clear();

	parse(filename, 1);
}

//-----------------------------------------------------------------------------------------------
void LevelItemTableManager::readline( std::stringstream& _stream )
{
	LevelItem* data = new LevelItem;
	data->readline(_stream);
	mLevelItemList.insert(LevelItemList::value_type(data->id, data));
}

//-----------------------------------------------------------------------------------------------
const LevelItem * LevelItemTableManager::getLevelItemByID( unsigned int id )
{
	if(mLevelItemList.find(id) != mLevelItemList.end())
		return mLevelItemList.find(id)->second;
	return NULL;
}

///====================================
void ResourceConfigManager::init( const std::string& filename )
{
	ResourceConfigList::iterator itr = mResourceConfigList.begin();
	while(itr != mResourceConfigList.end())
	{
		delete itr->second;
		itr->second =NULL;
		++itr;
	}
	mResourceConfigList.clear();

	parse(filename, 1);
}

ResourceConfigItem* ResourceConfigManager::getResourceItem( const std::string& platform )
{
	ResourceConfigList::iterator iter = mResourceConfigList.begin();
	for (; iter != mResourceConfigList.end(); ++iter)
	{
		if ((*iter->second).platform == platform)
		{
			return iter->second;
		}
	}
...
void ResourceConfigManager::readline( std::stringstream& _stream )
{
	ResourceConfigItem* data = new ResourceConfigItem;
	data->readline(_stream);
	mResourceConfigList.insert(ResourceConfigList::value_type(data->id, data));
}



///====================================
void I18nLanguageCfgManager::init(const std::string& filename)
{
	I18nConfigList::iterator itr = mI18nConfigList.begin();
	while (itr != mI18nConfigList.end())
	{
		delete itr->second;
		itr->second = NULL;
		++itr;
	}
	mI18nConfigList.clear();

	parse(filename, 1);
}

I18nLanguageCfgItem* I18nLanguageCfgManager::getI18nItem(ccLanguageType languageType)
{
	I18nConfigList::iterator iter = mI18nConfigList.begin();
	for (; iter != mI18nConfigList.end(); ++iter)
	{
		if ((*iter->second).languageType == languageType)
		{
			return iter->second;
		}
	}
...
void I18nLanguageCfgManager::readline(std::stringstream& _stream)
{
	I18nLanguageCfgItem* data = new I18nLanguageCfgItem;
	data->readline(_stream);
	mI18nConfigList.insert(I18nConfigList::value_type(data->id, data));
}
...
///=========I18nPlatform cfg
void I18nPlatformCfgItem::readline(std::stringstream& _stream)
{
	std::string str;
	_stream
		>> id
		>> platformName
		>> str;
	StringConverter::parseVector(str, languageTypeList);
}

void I18nPlatformCfgManager::init(const std::string& filename)
{
	I18nPlatformCfgList::iterator itr = mI18nPlatformCfgList.begin();
	while (itr != mI18nPlatformCfgList.end())
	{
		delete itr->second;
		itr->second = NULL;
		++itr;
	}
	mI18nPlatformCfgList.clear();

	parse(filename, 1);
}

I18nPlatformCfgItem* I18nPlatformCfgManager::getI18nPlatformItem(const std::string& platform)
{
	I18nPlatformCfgList::iterator iter = mI18nPlatformCfgList.begin();
	for (; iter != mI18nPlatformCfgList.end(); ++iter)
	{
		if ((*iter->second).platformName == platform)
		{
			return iter->second;
		}
	}
...
void I18nPlatformCfgManager::readline(std::stringstream& _stream)
{
	I18nPlatformCfgItem* data = new I18nPlatformCfgItem;
	data->readline(_stream);
	mI18nPlatformCfgList.insert(I18nPlatformCfgList::value_type(data->id, data));
}
//===========Tip.txt
void TipsItem::readline( std::stringstream& _stream )
{
	std::string str;
	_stream
		>> id
		>> tip;
	StringConverter::parseVector(str, tipList);
}

void TipsManager::init( const std::string& filename )
{
	TipsItemList::iterator itr = mTipsItemList.begin();
	while (itr != mTipsItemList.end())
	{
		delete itr->second;
		itr->second = NULL;
		++itr;
...
void TipsManager::readline( std::stringstream& _stream )
{
	TipsItem* data = new TipsItem;
	data->readline(_stream);
	mTipsItemList.insert(TipsItemList::value_type(data->id, data));
}
