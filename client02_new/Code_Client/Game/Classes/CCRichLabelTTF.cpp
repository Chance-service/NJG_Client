...
CCRichLabelTTF::~CCRichLabelTTF()
{
	for (std::map<CCObject*, unsigned int>::iterator it = m_itemInfo.begin(); it != m_itemInfo.end(); ++it)
	{
		it->first->release();
	}
	m_itemInfo.clear();
	for (std::map<unsigned int, CCLabelTTF*>::iterator it = m_LabelsInfo.begin(); it != m_LabelsInfo.end(); ++it)
	{
		it->second->release();
	}
	m_LabelsInfo.clear();
...
{
	std::map<CCObject*, unsigned int>::iterator find_it = m_itemInfo.find(pSender);
...
{
	std::map<unsigned int, CCLabelTTF*>::iterator find_it = m_LabelsInfo.find(index);
