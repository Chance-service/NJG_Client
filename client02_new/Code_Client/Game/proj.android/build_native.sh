...
export COCOS2DXTALKINGDATA_ROOT="$COCOS2DX_ROOT/external/TalkingDataGameAnalytics"
...
if [[ "$buildexternalsfromsource" ]]; then
    ...
    set -x
    "$NDK_ROOT"/android-ndk-r8e/ndk-build -C "$APP_ANDROID_ROOT" $* \
        "NDK_MODULE_PATH=${COCOS2DX_ROOT}:${COCOS2DXTALKINGDATA_ROOT}；${COCOS2DX_ROOT}/cocos2dx/platform/third_party/android/source"
else
    echo "Using prebuilt externals"
    set -x
    "$NDK_ROOT"/android-ndk-r8e/ndk-build -C "$APP_ANDROID_ROOT" -B $* \
        "NDK_MODULE_PATH=:${COCOS2DXTALKINGDATA_ROOT}；${APP_ROOT}/..:${COCOS2DX_ROOT}:${COCOS2DX_ROOT}/cocos2dx/platform/third_party/android/prebuilt"
fi
