...
TOLUA_API int toluafix_ref_table(lua_State* L, int lo, int def)
{
	...
	lua_pushstring(L, TOLUA_REFID_TABLE_MAPPING);
	lua_rawget(L, LUA_REGISTRYINDEX);                           /* stack: fun ... refid_table */
	lua_pushinteger(L, s_table_ref_id);                      /* stack: fun ... refid_table refid */
	lua_pushvalue(L, lo);                                       /* stack: fun ... refid_table refid table */

	lua_rawset(L, -3);                  /* refid_table[refid] = table, stack: table ... refid_ptr */
...
TOLUA_API void toluafix_get_table_by_refid(lua_State* L, int refid)
{
	...
	lua_rawget(L, -2);                                          /* stack: ... refid_table table */
...
TOLUA_API void toluafix_remove_table_by_refid(lua_State* L, int refid)
{
	...
	lua_rawset(L, -3);                  /* refid_table[refid] = table, stack: ... refid_ptr */
	lua_pop(L, 1);                                              /* stack: ... */
