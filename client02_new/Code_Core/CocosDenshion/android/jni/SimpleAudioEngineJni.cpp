...
public:
    void update(float dt)
    {
        ...
        {
			//LOGD("dt:%f", dt);
			if (iter->second._playOver && iter->second._timer >= 1.0f)
            {
				LOGD("_playOver path : %s", iter->second._path.c_str());
                s_audioPlayers.erase(iter);
                break;
            }
			if (iter->second._delayRemove && iter->second._timer >= 1.0f)
			{
				LOGD("_delayRemove path : %s", iter->second._path.c_str());
				//SLresult result = (*iter->second._fdPlayerPlay)->SetPlayState(iter->second._fdPlayerPlay, SL_PLAYSTATE_STOPPED);
				s_audioPlayers.erase(iter);
				break;
			}
			if (iter->second._delayRemove || iter->second._playOver) {
				iter->second._timer = iter->second._timer + 1.0f;
			}
        }
    }
};
...
extern "C"
{
	...
	void playOtherBackgroundMusicJNI(const char *path, bool isLoop)
	{
		// void playBackgroundMusic(String,boolean)

		JniMethodInfo methodInfo;
		LOGD("-------playOtherBackgroundMusicJNI");
		if (!getStaticMethodInfo(methodInfo, "playOtherBackgroundMusic", "(Ljava/lang/String;Z)V"))
		{
			LOGD("-------playOtherBackgroundMusicJNI2");
			return;
		}
		LOGD("-------playOtherBackgroundMusicJNI3");
		jstring stringArg = methodInfo.env->NewStringUTF(path);
		LOGD("-------playOtherBackgroundMusicJNI4");
		methodInfo.env->CallStaticVoidMethod(methodInfo.classID, methodInfo.methodID, stringArg, isLoop);
		LOGD("-------playOtherBackgroundMusicJNI5");
		methodInfo.env->DeleteLocalRef(stringArg);
		LOGD("-------playOtherBackgroundMusicJNI6");
		methodInfo.env->DeleteLocalRef(methodInfo.classID);
		LOGD("-------playOtherBackgroundMusicJNI7");
	}
	...
	void stopOtherBackgroundMusicJNI()
	{
		// void stopOtherBackgroundMusic()

		JniMethodInfo methodInfo;

		if (!getStaticMethodInfo(methodInfo, "stopOtherBackgroundMusic", "()V"))
		{
			return;
		}
	...
	bool isOtherBackgroundMusicPlayingJNI()
	{
		// boolean rewindBackgroundMusic()
		LOGD("-------isOtherBackgroundMusicPlayingJNI");
		JniMethodInfo methodInfo;
		jboolean ret = false;
		LOGD("-------isOtherBackgroundMusicPlayingJNI2");
		if (!getStaticMethodInfo(methodInfo, "isOtherBackgroundMusicPlaying", "()Z"))
		{
			LOGD("-------isOtherBackgroundMusicPlayingJNI3");
			return ret;
		}
		LOGD("-------isOtherBackgroundMusicPlayingJNI4");
		ret = methodInfo.env->CallStaticBooleanMethod(methodInfo.classID, methodInfo.methodID);
		LOGD("-------isOtherBackgroundMusicPlayingJNI5");
		methodInfo.env->DeleteLocalRef(methodInfo.classID);
		LOGD("-------isOtherBackgroundMusicPlayingJNI6");
		return ret;
	}
...
void stopAllEffectsJNI()
{
    ...
    for (AudioPlayerIt it = s_audioPlayers.begin(); it != itEnd; ++it)
    {
		...
		it->second._delayRemove = true;
		it->second._timer = 0.0f;
    }
