...
static void reSetTouches(CCSet &set)
{
	s_indexBitsUsed = 0;
...
void CCEGLViewProtocol::setDesignResolutionSize(float width, float height, ResolutionPolicy resolutionPolicy)
{
    ...
    if (resolutionPolicy == kResolutionNoBorder)
    {
		...
		//if (m_fScaleX>2 && m_fScaleX<3)
		//{
		//	//m_fScaleX = 2;
		//}
		//CCLog("m_obScreenSize.width %f m_obDesignResolutionSize.width%f m_fScaleX %f  ", m_obScreenSize.width, m_obDesignResolutionSize.width, m_fScaleX);
		//CCLog("m_obScreenSize.height %f m_obDesignResolutionSize.height%f m_fScaleY %f  ", m_obScreenSize.height, m_obDesignResolutionSize.height, m_fScaleY);
    } 
	...
	float viewPortW = m_obScreenSize.width;// m_obDesignResolutionSize.width * m_fScaleX;
	float viewPortH = m_obScreenSize.height; //m_obDesignResolutionSize.height * m_fScaleY;

    m_obViewPortRect.setRect((m_obScreenSize.width - viewPortW) / 2, (m_obScreenSize.height - viewPortH) / 2, viewPortW, viewPortH);
	//CCLog("width %f height %f   viewPortW%f  viewPortH %f ", m_obViewPortRect.size.width, m_obViewPortRect.size.height, viewPortW, viewPortH);
...
static unsigned int lockcount = 0;
void CCEGLViewProtocol::lockTouches()
{
	if(lockcount == 0)
		m_bTouchesLock = true;
	lockcount++;
	...
	CCLog("CCEGLViewProtocol::lockTouches,lockcount = %d",lockcount);
}

void CCEGLViewProtocol::unlockTouches()
{
	if(lockcount > 0)
	{
		lockcount--;
	...
	CCLog("CCEGLViewProtocol::unlockTouches,lockcount = %d",lockcount);
}

bool CCEGLViewProtocol::isTouchesLocked()
{
	return m_bTouchesLock;
}
