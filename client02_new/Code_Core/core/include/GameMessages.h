...
{
	RETURN_TRUE_TYPE(MsgRefreshRedPoint)
		MsgRefreshRedPoint(){ pageName = ""; extraParam = ""; };
	std::string pageName;
	std::string extraParam;
	//virtual std::string getParameters(){return pageName;}
	virtual const unsigned int getTypeId() const{ return MSG_REFRESH_REDPOINT; }
	virtual GameMessage * clone() const{ MsgRefreshRedPoint * ret = new MsgRefreshRedPoint; ret->pageName = pageName; ret->extraParam = extraParam; return ret; }
};

struct MsgSendPackageFailed : public GameMessage
{
	RETURN_TRUE_TYPE(MsgSendPackageFailed)
		MsgSendPackageFailed(){ opcode = 0; };
	unsigned int opcode;
	//virtual std::string getParameters(){return pageName;}
	virtual const unsigned int getTypeId() const{ return MSG_SEND_PACKAGE_FAILED; }
	virtual GameMessage * clone() const{ MsgSendPackageFailed * ret = new MsgSendPackageFailed; ret->opcode = opcode; return ret; }
};