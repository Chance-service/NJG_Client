...
// 	int n 	/* how many chars to process */
	...
	static std::string stringAutoReturnForLuaNewApi(const std::string& inString, int width, int& lines);
	...
	// hash utils
	static std::string calMd5(FILE* fp, char* buf, size_t buf_size, double* pnow = NULL, double* ptotal = NULL);
};
