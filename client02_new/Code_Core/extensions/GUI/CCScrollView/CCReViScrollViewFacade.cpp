...
cocos2d::extension::CCReViScrollViewFacade::CCReViScrollViewFacade( CCScrollView* pView )
	...
	,m_pOtherDelegate(NULL),m_bouncedFlag(false)
{
...
#define  SV_BOUNCED_TIME 0.2f

void cocos2d::extension::CCReViScrollViewFacade::scrollViewDidDeaccelerateStop(CCScrollView* view, CCPoint initSpeed)
{
	if (m_bouncedFlag)
	{
		CCPoint curPoint = view->getContentOffset();
		CCSize firstChildSize =  this->getFirstChildContentSize();
		int width = firstChildSize.width;
		int height = firstChildSize.height;
		//暂时只支持Vertical的scrollView
		if (height>0 &&width>0 &&view->getDirection() == kCCScrollViewDirectionVertical )
		{
			float curOffsetY = curPoint.y;
			int viewHeight = view->getViewSize().height;
			//step.1 calculate the indexUp and indexDown based on the curOffsetY
			int indexUp = ceil((viewHeight - curOffsetY)/height);
			int indexDown = floor((viewHeight - curOffsetY)/height);
			int maxCount = m_arrayItems->count();
			int minCount = 1;
			//step.2 clamp indexUp and indexDown
			if (indexUp<minCount)
				indexUp =minCount;
			if (indexUp>maxCount)
				indexUp =maxCount;
			if (indexDown<minCount)
				indexDown =minCount;
			if (indexDown>maxCount)
				indexDown =maxCount;
			//step.3 calculate new upOffset and downOffset based on the indexUp and indexDown
			float upOffsetY = viewHeight - indexUp * height;
			float downOffsetY = viewHeight - indexDown * height;
			//step.4 choose the nearest offsetY 
			float minusUp = abs(curOffsetY-upOffsetY);
			float minusDown = abs(curOffsetY-downOffsetY);
			float newOffsetY = upOffsetY;
			if (minusUp<minusDown)
			{
				newOffsetY = upOffsetY;
			} 
			else
			{
				newOffsetY = downOffsetY;
			}
			//step.5 stop other action and run the new offset action, then refresh
			view->getContainer()->stopAllActions();
			view->setContentOffsetInDuration(ccp(view->getContentOffset().x, newOffsetY),SV_BOUNCED_TIME);
			this->refreshDynamicScrollView();
		}
	}
	if (m_pOtherDelegate)
...
cocos2d::CCSize cocos2d::extension::CCReViScrollViewFacade::getFirstChildContentSize()
{
	...
	if (children)
	{
		...
		if (count>0)
		{
			CCNode* child =  (CCNode*)children->objectAtIndex(0);
			if (child)
			{
...
void cocos2d::extension::CCReViScrollViewFacade::setAndRefreshItemsByIndex(int index)
{
	...
	if (index>maxCount)
		index =maxCount;
	this->setDynamicItemsStartPosition(index-1);
	int viewHeight = m_pScrollView->getViewSize().height;
	CCSize firstChildSize =  this->getFirstChildContentSize();
	int width = firstChildSize.width;
	int height = firstChildSize.height;
	if (width>0 && height>0 && viewHeight>0)
	{
		float newOffsetY = viewHeight - index*height;
		if (newOffsetY>0)
		{
			newOffsetY = 0;
		}
		m_pScrollView->getContainer()->stopAllActions();
		m_pScrollView->setContentOffsetInDuration(ccp(m_pScrollView->getContentOffset().x, newOffsetY),0.2f);
	}
	
}


void cocos2d::extension::CCReViScrollViewFacade::setAndRefreshItemsByOffset(CCPoint offset)
{
	//step.1 set position of the scrollView
	if(m_pScrollView){
		m_pScrollView->getContainer()->stopAllActions();
		m_pScrollView->setContentOffsetInDuration(offset,0.2f);
		//m_pScrollView->getContainer()->setPosition(offset);
		CCSize firstChildSize =  this->getFirstChildContentSize();
		int width = firstChildSize.width;
		int height = firstChildSize.height;
		if (width>0 && height>0)
		{
			float curOffsetY = offset.y;
			int viewHeight = m_pScrollView->getViewSize().height;
			//step.1 calculate the indexUp and indexDown based on the curOffsetY
			int indexUp = ceil((viewHeight - curOffsetY)/height);
			int maxCount = m_arrayItems->count();
			...
			if (indexUp>maxCount)
				indexUp =maxCount;
			this->setDynamicItemsStartPosition(indexUp-1);
		}
	}
}
...
void cocos2d::extension::CCReViScrollViewFacade::addItem( CCReViSvItemData* item, CCReViSvItemNodeFacade* node /*= NULL*/ )
{
	...
	if (node && node->m_pItemNode && (m_iFixedViewItemsMaxNum*Macro_FixedItemViewsMultiply > m_arrayItemViews.size()))
	{
		...
		m_pScrollView->addChild(node->m_pItemNode);
