...
CCNode* CCBReader::readNodeGraphFromData(CCData *pData, CCObject *pOwner, const CCSize &parentSize)
{
	...
	if (!mOwnerOutletNodes)
	{
		mOwnerOutletNodes = new CCArray();
	}
   
	if (!mOwnerCallbackNodes)
	{
		mOwnerCallbackNodes = new CCArray();
	}
...
bool CCBReader::readHeader()
{
    ...
    if(version != kCCBVersion) {
		char msgStr[512] = {0};
		sprintf(msgStr, "WARNING! Incompatible ccbi file: %s version (file: %d reader: %d)", this->mCCBName.c_str(), version, kCCBVersion);
		CCLog(msgStr);
		CCAssert(false, msgStr);
        return false;
    }
    ...
    else if (type == kCCBPropTypeSpriteFrame)
    {
        ...
        if (spriteSheet.length() == 0)
        {
			...
			if(!spriteFrame)
			{
				...
				if (texture == NULL) texture = CCTextureCache::sharedTextureCache()->addImage("UI/Mask/Image_Empty.png");
