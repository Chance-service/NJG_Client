...
CCActionInterval* CCBAnimationManager::getAction(CCBKeyframe *pKeyframe0, CCBKeyframe *pKeyframe1, const char *pPropName, CCNode *pNode)
{
    ...
    {
        CCBValue *value = (CCBValue*)pKeyframe1->getValue();
		GLubyte byteValue =  value->getByteValue();
        return CCFadeTo::create(duration, byteValue);
    }
	...
	else if (strcmp(pPropName, "mask") == 0)
	{
		return CCSequence::createWithTwoActions(CCDelayTime::create(duration),
			CCBSetStencil::create((CCSpriteFrame*)pKeyframe1->getValue()));
	}
    else 
    {
    ...
    else 
    {
        ...
        else 
        {
            ...
            else if (strcmp(pPropName, "opacity") == 0)
            {
                ...
                CCRGBAProtocol* rgbNode = (dynamic_cast<CCRGBAProtocol*>(pNode));
				if (rgbNode)
				{
					rgbNode->setOpacity(opacity);
				}else
				{
					CCParticleSystemQuad* quadNode = dynamic_cast<CCParticleSystemQuad*>(pNode);
					if (quadNode)
					{
						quadNode->setOpacity(opacity);
					}
				}
            }
            else if (strcmp(pPropName, "displayFrame") == 0)
			...
			{
				(dynamic_cast<CCBClippingNode*>(pNode))->setStencil(CCSprite::createWithSpriteFrame((CCSpriteFrame*)pValue));
			}
            else
            {
...
bool CCBAnimationManager::hasAnimation(const char* animation)
{
	return getSequenceId(animation)>=0;
}

float CCBAnimationManager::getAnimationLength( const char* animation )
{
	CCObject *pElement = NULL;
	string seqName(animation);
	CCARRAY_FOREACH(mSequences, pElement)
	{
		CCBSequence *seq = (CCBSequence*)pElement;
		if (seqName.compare(seq->getName()) == 0)
...
CCBSetStencil* CCBSetStencil::create(CCSpriteFrame *pSpriteFrame)
{
    CCBSetStencil *ret = new CCBSetStencil();
    if (ret)
    {
        if (ret->initWithSpriteFrame(pSpriteFrame))
        {
...
bool CCBSetStencil::initWithSpriteFrame(CCSpriteFrame *pSpriteFrame)
{
    mSpriteFrame = pSpriteFrame;
    CC_SAFE_RETAIN(mSpriteFrame);
...
CCBSetStencil::~CCBSetStencil()
{
    CC_SAFE_RELEASE_NULL(mSpriteFrame);
}

CCObject* CCBSetStencil::copyWithZone(CCZone *pZone)
{
    CCZone *pNewZone = NULL;
    CCBSetStencil *pRet = NULL;
    
    if (pZone && pZone->m_pCopyObject) {
        pRet = (CCBSetStencil*) (pZone->m_pCopyObject);
    } else {
        pRet = new CCBSetStencil();
        pZone = pNewZone = new CCZone(pRet);
    }
...
void CCBSetStencil::update(float time)
{
	CCBClippingNode* target = dynamic_cast<CCBClippingNode*>(m_pTarget);
	if (target && mSpriteFrame)
	{
		target->setStencil(CCSprite::createWithSpriteFrame(mSpriteFrame));
	}
}
/************************************************************
 CCBSoundEffect
