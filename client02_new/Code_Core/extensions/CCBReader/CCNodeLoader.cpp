...
//                 }
			...
			case kCCBPropTypeCCBFileNew: {
				CCNode * ccbFileNode = this->parsePropTypeCCBFileNew(pNode, pParent, pCCBReader);
				if(setProp) {
					this->onHandlePropTypeCCBFileNew(pNode, pParent, propertyName.c_str(), ccbFileNode, pCCBReader);
				}
				break;
			}
            default:
            ...
            if(target != NULL) {
                if(selectorName.length() > 0) {
                    ...
                    if(targetAsCCBSelectorResolver != NULL) {
						...
						if (!selCCControlHandler)
						{
							selCCControlHandler = targetAsCCBSelectorResolver->onResolveCCBCCControlSelectorWithSender(target, selectorName.c_str(),pNode);
						}
                    }
...
CCNode * CCNodeLoader::parsePropTypeCCBFileNew(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader) {
	std::string ccbFileName = pCCBReader->getCCBRootPath() + pCCBReader->readCachedString();
    ...
    CCLOG("ccFileName: %s", ccbFileName.c_str());

	// Load sub file
	CCBFileNew* ccbFileNode = dynamic_cast<CCBFileNew*>(pNode);
	// Handle sub ccb files (remove middle node)
	if (!ccbFileNode)
	{
		ccbFileNode = CCBFileNew::create();
	}

	ccbFileNode->setCCBFileName(ccbFileName);

	CCBFileNew* node = dynamic_cast<CCBFileNew*>(pCCBReader->getOwner());
	if(node)
	{
		ccbFileNode->setParentCCBFileNode(node);
		ccbFileNode->setListener(node->getListener());
	}
...
void CCNodeLoader::onHandlePropTypeCCBFileNew(CCNode * pNode, CCNode * pParent, const char* pPropertyName, CCNode * pCCBFileNode, CCBReader * pCCBReader) {
	ASSERT_FAIL_UNEXPECTED_PROPERTY(pPropertyName);
}
