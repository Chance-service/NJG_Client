...
#include <algorithm>
#include "../core/include/GameMessages.h"

NS_CC_EXT_BEGIN;
UpdateNode updateNodeHandle = NULL;
void setUpdateNodeHandle( UpdateNode handle)
{
	updateNodeHandle = handle;
}
...
void CCBFileNew::onEnter()
{
	CCNode::onEnter();
	if (this->hasAnimation("Normal"))
	{
		this->runAnimation("Normal");
	}
}
...
void CCBFileNew::load( bool froceLoad/* = false*/ )
{
	...
	if(mLoadCCBFile.empty())
		return;
	CCLog("LoadCcbi file: %s", mLoadCCBFile.c_str());
	...
	/* Read a ccbi file. */
	mLoadCCBFileNode = ccbReader->readNodeGraphFromFile(CCFileUtils::sharedFileUtils()->fullPathForFilename(mLoadCCBFile.c_str()).c_str(),this);
	...
	if(mLoadCCBFileNode)
	{
		if (updateNodeHandle)
		{
			updateNodeHandle(mLoadCCBFileNode);
		}
	...
	{
		char msgStr[512]={0};
		sprintf(msgStr,"CCBContainer.loadCcbiFile %s Failed",mLoadCCBFile.c_str());
	}
// 	addChild(mLoadCCBFileNode);
// 	setContentSize(mLoadCCBFileNode->getContentSize());
}
...
#define getVariableAs(_type_,_func_) \
_type_* CCBFileNew::_func_( const std::string& _var_ ) \
{ \
...
getVariableAs(CCNode,getVarNode);
getVariableAs(CCSprite,getVarSprite);
getVariableAs(CCLabelBMFont,getVarLabelBMFont);
getVariableAs(CCLabelTTF,getVarLabelTTF);
getVariableAs(CCBFileNew,getVarCCNFileNew);
getVariableAs(CCMenu,getVarMenu);
getVariableAs(CCMenuItem,getVarMenuItem);
getVariableAs(CCMenuItemSprite,getVarMenuItemSprite);
getVariableAs(CCMenuItemImage,getVarMenuItemImage);
getVariableAs(CCScrollView,getVarScrollView);
getVariableAs(CCScale9Sprite,getVarScale9Sprite);
getVariableAs(CCParticleSystem,getParticleSystem);
getVariableAs(CCMenuItemCCBFile,getVarMenuItemCCB);
getVariableAs(CCLayer,getVarLayer);
...
void CCBFileNew::_menuItemClicked( CCObject * pSender )
{
	...
	if(it!=mMenus.end())
	{
		...
		MessageManager::Get()->sendMessageImmediately(&msg);
...
extension::SEL_CCControlHandler CCBFileNew::onResolveCCBCCControlSelectorWithSender(CCObject * pTarget, const char* pSelectorName, CCNode* sender)
{
	if(pTarget!=this)
		return NULL;
	std::string name = pSelectorName;
	...
	return cccontrol_selector(CCBFileNew::_menuItemClickedForCCControl);
}

void CCBFileNew::_menuItemClickedForCCControl(CCObject * pSender,unsigned int eventType)
{
	MENUITEM_MAP::iterator it = mMenus.find(pSender);
	if(it!=mMenus.end())
	{
		...
		//CCLOG("CCBFileNew:Control Event! ccb:%s, func:%s, tag:%s",mLoadCCBFile.c_str(),it->second.c_str(),eventName);
		onContolAction(it->second,pSender,eventName);
	}
}
...
void CCBFileNew::onContolAction(const std::string& itemName, CCObject* sender, std::string tag)
{
	if(!Run_Script_Fun(itemName))
	{
	/*	CCBSoundBehavior soundBehavior(CCB_PLAY_SOUND,mLoadCCBFile+"#"+itemName,CCBSoundBehavior::PRESS);
		...
		if(mCCBFileListener)
			mCCBFileListener->onContolAction(itemName, sender, tag);
		if(mParentCCBFileNode)
			mParentCCBFileNode->onContolAction(itemName,sender,tag);
	}
}
...
CCBFileNew* CCBFileNew::CreateInPool( const std::string& CCBFile)
{
	CCBFileNew* pRet = NULL;

	std::map<std::string,std::list<CCBFileNew*> >::iterator mapit = ccbsPool.find(CCBFile);
	if(mapit==ccbsPool.end())
	{
		pRet = new CCBFileNew();
		pRet->mLoadCCBFile = CCBFile;
		std::list<CCBFileNew*> ccbclist;
		ccbclist.push_back(pRet);
		pRet->mIsInPool = true;
		pRet->load();
		ccbsPool.insert(std::make_pair(CCBFile,ccbclist));
		return pRet;
	}
	else
	{
       ...
       CCLog("new ccbi %s loaded:%d",CCBFile.c_str(),mapit->second.size());
		pRet = new CCBFileNew();
		pRet->mLoadCCBFile = CCBFile;
