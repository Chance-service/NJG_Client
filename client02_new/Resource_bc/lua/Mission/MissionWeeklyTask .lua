...
local thisPageName = 'MissionWeelyTask'
...
require("Shop_pb");
local MissionWeelyTask = {
}
local WeeklyQuestCfg = { }-- 任务配置
local WeeklyQuestPointCfg = { }-- 活跃点配置
local _dailyQuestPacketInfo = { }-- 任务数据包
local _curPoint = 0-- 当前活跃点
local _LastPoint = 0-- 记录上次活跃点 用于做动画
local _weeklyPointCore = { }-- 活跃度状态信息
...
function MissionWeelyTask.onFunction(eventName, container)
    if eventName:sub(1, 8) == "onGetBox" then
        local curBoxId = tonumber(string.sub(eventName, -1, -1))
        local curState = _weeklyPointCore[curBoxId].state
        if curState == 2 then
        ...
        elseif curState == 0 then
            ...
            msg.pointCount = _weeklyPointCore[index].dailyPointNumber
            local pb = msg:SerializeToString()
            PacketManager:getInstance():sendPakcet(HP_pb.TAKE_WEEKLY_QUEST_POINT_AWARD_C, pb, #pb, true)
...
function MissionWeelyTask:onEnter(ParentContainer)
    _LastPoint = 0
    _isNeedRunAni = false
    ...
    selfContainer:registerFunctionHandler(MissionWeelyTask.onFunction)
    ...
    WeeklyQuestCfg = ConfigManager.getWeeklyQuestCfg()
    WeeklyQuestPointCfg = ConfigManager.getWeeklyQuestPointCfg()

    return selfContainer
end
function MissionWeelyTask:initProgressTimer(container)
    ...
    --for i = 1, #WeeklyQuestPointCfg do
    --    local cfg = WeeklyQuestPointCfg[i]
...
function MissionWeelyTask:getPacketInfo()
    common:sendEmptyPacket(HP_pb.WEEKLY_QUEST_INFO_C, true)
end
function MissionWeelyTask:refreshPage()
    ...
    for i = 1, #_weeklyPointCore do
        NewPos[i]=_weeklyPointCore[i].dailyPointNumber
    end
   ...
   if _curPoint ~= _LastPoint then
    ...
    for i = 1, #_weeklyPointCore do
        ...
        --
        for i = 1, #_weeklyPointCore do
            strLabel["mBoxNum" .. i] = _weeklyPointCore[i].dailyPointNumber
            NodeHelper:setSpriteImage(selfContainer,{["mBoxPic"..i] = "DayLogin30_treasurebox2.png"})
            for j = 0, 2 do
                local curName = "mTaskRewardBox" .. j .. "_" .. spriteName[i]
                local sprite = selfContainer:getVarNode(curName);
                if sprite then
                    sprite:setVisible(j == _weeklyPointCore[i].state)             
                end
                if j == 0 or j == 1 then
                    NodeHelper:setSpriteImage(selfContainer,{[curName] = "DayLogin30_treasurebox2.png"})
                else
                    NodeHelper:setSpriteImage(selfContainer,{[curName] = "DayLogin30_treasurebox2_open.png"})
                end
                NodeHelper:setNodesVisible(selfContainer, { ["mBoxPoint" .. i] = (1 == _weeklyPointCore[i].state) })
            end
        end
    end
    NodeHelper:setStringForLabel(selfContainer, strLabel);
    --MissionWeelyTask:onfreshSignState()
...
function  MissionWeelyTask:onfreshSignState()
...
function TaskContent:onConfirmation(content)
    ...
    local cfgInfo = WeeklyQuestCfg[packetInfo.questId]
...
function TaskContent.getRewards(id)
    ...
    msg.questId = id
    local pb = msg:SerializeToString()
    PacketManager:getInstance():sendPakcet(HP_pb.TAKE_WEEKLY_QUEST_AWARD_C, pb, #pb, true)   
end
function TaskContent:onRefreshContent(content)
    local container = content:getCCBFileNode()
    local index = self.id
    local ItemInfo = _dailyQuestPacketInfo[index]
    local ItemCfg = WeeklyQuestCfg[ItemInfo.questId]
...
function MissionWeelyTask:rebuildAllItem()
    self:clearAllItem();
    self:buildItem();
end

function MissionWeelyTask:clearAllItem()
    local scrollview = selfContainer.scrollview
    scrollview:removeAllCell();
end

function MissionWeelyTask:buildItem()
...
function MissionWeelyTask:onBuyBtn(container)


end

function MissionWeelyTask:getActivityInfo()
...
local function sortInfoList(info)
    ...
    function(e1, e2)
        ...
        local task1Cfg = WeeklyQuestCfg[e1.questId]
        local task2Cfg = WeeklyQuestCfg[e2.questId]
        -- sortId
...
local function sortQuestInfo(info)
    ...
    for i = 1, #info do
        local ItemCfg = WeeklyQuestCfg[info[i].questId];
...
function MissionWeelyTask:onReceivePacket(ParentContainer)
    local opcode = ParentContainer:getRecPacketOpcode();
    local msgBuff = ParentContainer:getRecPacketBuffer();
    if opcode == HP_pb.WEEKLY_QUEST_INFO_S or opcode == HP_pb.TAKE_WEEKLY_QUEST_AWARD_S then
        if opcode == HP_pb.TAKE_WEEKLY_QUEST_AWARD_S then
            _isNeedRunAni = true
        ...
        _weeklyPointCore = msg.dailyPointCore


        --------------------------------------------------------------------------
        --local index = 0
        --for i = 1 , #_weeklyPointCore do
        --    if _weeklyPointCore[i].dailyPointNumber == 100 then
        --       index = i
        --       break
        --    end
        --end
        --if index ~= 0 then
        --  table.remove(_weeklyPointCore , index)
        --end
        --------------------------------------------------------------------------


        table.sort(_weeklyPointCore, function(point1, point2)
        ...
        self:refreshPage()
    elseif opcode == HP_pb.TAKE_WEEKLY_QUEST_POINT_AWARD_S then
...
function MissionWeelyTask:onReceiveMessage(ParentContainer)
...
function MissionWeelyTask:removePacket(ParentContainer)
    ParentContainer:removeMessage(MSG_MAINFRAME_REFRESH);
end
function MissionWeelyTask:onExecute(container)
    if goBattlePage == true then
        goBattlePage = false
--        MainFrame_onBattlePageBtn();
    end
end
function MissionWeelyTask:onExit(ParentContainer)
    _ProgressTimerNode = nil
end

return MissionWeelyTask
