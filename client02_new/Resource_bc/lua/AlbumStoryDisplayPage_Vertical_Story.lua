local thisPageName = "AlbumStoryDisplayPage_Vertical_Story"
...
local AlbumStoryDisplayBase_Vertical_Story = {}
local SpineData = {}
local SpineIdx = 1
local Spines = {}
local spineNode = {}
...
local soundTable = {}
--動畫資料
function AlbumStoryDisplayBase_Vertical_Story:setData(mID,isStory)
    local Tmptable = {}
    local cfg = ConfigManager.getStoryData_Vertical_StorySpine()
    for i = 1, 99 do
    ...
    for _,data in pairs (SpineData) do
        local nameTable = common:split(data.Voice,",")
        for _,v in ipairs (nameTable) do
            local filename = common:split(v,"_")[3] 
            table.insert(soundTable,filename)
        end     
    end
    AlbumStoryDisplayBase_Vertical_Story:preloadAudioEffects(soundTable,3)
    Story=isStory
end

function AlbumStoryDisplayBase_Vertical_Story:preloadAudioEffects(files, batchSize)
...
function AlbumStoryDisplayBase_Vertical_Story:onEnter(container)
    isAuto = false
    TouchCD = false
    logIndex = 1
    logTable = {}
    ...
    --AlbumStoryDisplayBase_Vertical_Story:initSetting(selfContainer)
...
--首次spine
function AlbumStoryDisplayBase_Vertical_Story:setSpine(container)
    Spines = {}
    spineNode = {}
    SpineIdx = 1
    local spineNames = { }
    ...
    for i = 1, #SpineData do
        spineNames[i] = SpineData[i].Spine
        if not spineNames[i - 1] or spineNames[i - 1] ~= spineNames[i] then
            Spines[i] = SpineContainer:create(spinePath, SpineData[i].Spine)
            Spines[i]:registerFunctionHandler("COMPLETE", AlbumStoryDisplayBase_Vertical_Story.onFunction)
        else
            Spines[i] = Spines[i - 1]
        end
...
function AlbumStoryDisplayBase_Vertical_Story:SetNextSpine(container)
...
--第一次進動畫
function AlbumStoryDisplayBase_Vertical_Story:initSetting(container)
    lines = {}
    linesCharater = {}
    nowLineTable = {}
    NodeHelper:setStringForLabel(container, {mTxt = ""})
    ...
    --首次播配音
    AlbumStoryDisplayBase_Vertical_Story:playVoice(container)
    --首次播音效
    ...
    --            SoundManager:getInstance():playEffectByName(newOtherEffName,false)
    --        end))
...
--動畫完成一次播放
function AlbumStoryDisplayBase_Vertical_Story:onFunction(tag, eventName)
...
--玩家點擊
function AlbumStoryDisplayBase_Vertical_Story:onTouch(container, eventName)
...
--配音播放
function AlbumStoryDisplayBase_Vertical_Story:playVoice(container)
...
--停止音效
function AlbumStoryDisplayBase_Vertical_Story:stopVoice(container)
    SoundManager:getInstance():stopAllEffect()
...
--退出
function AlbumStoryDisplayBase_Vertical_Story:onReturn(container,Finish)
    isAuto = false
    if Finish == true then
        AlbumStoryDisplayBase_Vertical_Story:Quit()
    else
...
function AlbumStoryDisplayBase_Vertical_Story:Quit()
    AlbumStoryDisplayBase_Vertical_Story:unloadAudioEffects(soundTable, 5)
    local spinePath = "Spine/NGUI"
    local spineName = "NGUI_77_Shady"
    local spine = SpineContainer:create(spinePath, spineName)
...
function AlbumStoryDisplayBase_Vertical_Story:unloadAudioEffects(files, batchSize)
    for i = 1, #files, batchSize do
        local endIndex = math.min(i + batchSize - 1, #files)
        for j = i, endIndex do
...
--下一個Spine
function AlbumStoryDisplayBase_Vertical_Story:onNext()
    SpineIdx = SpineIdx + 1
    if SpineIdx <= #Spines then
        --if not spineNode[SpineIdx - 1] or spineNode[SpineIdx - 1] ~= spineNode[SpineIdx] then
        --    parentNode:removeAllChildrenWithCleanup(true)
        ...
        --PageManager.popPage(thisPageName)
        NodeHelper:setNodesVisible(selfContainer, {mReturnBtn = true})
        AlbumStoryDisplayBase_Vertical_Story:onReturn(selfContainer,true)
    end
end
function AlbumStoryDisplayBase_Vertical_Story:onExecute(container)
    if mSpineNode then
        mSpineNode:scheduleUpdateWithPriorityLua(function(dt)
            AlbumStoryDisplayBase_Vertical_Story:update(dt, container)
        end, 0)
    end
end
--播放文字
function AlbumStoryDisplayBase_Vertical_Story:update(dt, container)
    if nowLineTable and nowLineTable[lineIndex] then
        ...
        if not nowLineTable[lineIndex] then
            isLabelPlaying = false
            if SpineData[SpineIdx] and SpineData[SpineIdx].type == 0 then
                NodeHelper:setNodesVisible(container, {mSpine3 = false})
            else
                if not isHide then
                    NodeHelper:setNodesVisible(container, {mSpine3 = true})
...
----AUTO
function AlbumStoryDisplayBase_Vertical_Story:onAuto(container)
    if isAuto then
        isAuto = false
        TouchCD = false
...
function AlbumStoryDisplayBase_Vertical_Story:SelfTouch(container, eventName)
    if nowState == DiaryState.SPINE then
        if isLabelPlaying then
            isLabelPlaying = false
...
--HIDE
function AlbumStoryDisplayBase_Vertical_Story:onHide(container)
...
----------LOG--------
function AlbumStoryDisplayBase_Vertical_Story:onLog(container) --開啟Log
    isLog=true
...
function AlbumStoryDisplayBase_Vertical_Story:clearAndReBuildAllLog(container)
    ...
    for i = 1, #FetterLogContent.logTxt do
        ...
        local height = AlbumStoryDisplayBase_Vertical_Story:calStringHeight(FetterLogContent.logTxt[i]) + 100
...
function AlbumStoryDisplayBase_Vertical_Story:calStringHeight(msg)
...
function AlbumStoryDisplayBase_Vertical_Story:setAllFetterLogContent(container)
...
function AlbumStoryDisplayBase_Vertical_Story:onExitBook(container)
    isLog = false
...
local AlbumStoryDisplayPage_Flip = CommonPage.newSub(AlbumStoryDisplayBase_Vertical_Story, thisPageName, option)
