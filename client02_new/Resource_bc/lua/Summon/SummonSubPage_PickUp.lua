...
--[[ 
    name: SummonSubPage_PickUp
    desc: 召喚 子頁面 PickUp召喚
    author: youzi
...
local VOUCHER_ITEM_STR = "30000_6012_0"
...
function Inst:onReceivePacket(packet)
    ...
    if self:handleSummonError(packet, HP_pb.ACTIVITY172_PICKUP_DRAW_C) == true then return end

    if packet.opcode == HP_pb.ACTIVITY172_PICKUP_INFO_S or
       packet.opcode == HP_pb.ACTIVITY172_PICKUP_DRAW_S then
        if packet.opcode == HP_pb.ACTIVITY172_PICKUP_INFO_S then
            print("ACTIVITY172_PICKUP_INFO_S")
        end
...
function Inst:onEnter_content ()
    ...
    if self.subPageCfg.spineBG ~= nil then
        ...
        NodeHelper:setSpriteImage(self.container,{mSprite = Cfg.Banner})
        local spineFolderAndName = common:split(Cfg.Spine, ",")
    ...
    --設置小人spine
    local Cfg=self:getCfg()
    local spineFolderAndName = common:split(Cfg.littleSpine, ",")
...
function Inst:loadSummonSpine ()
    ...
    if not self.spineSummon and self.subPageCfg.spineSummon ~= nil then
        local Cfg = self:getCfg()
        local spineFolderAndName = common:split(Cfg.EFF, ",")
...
function Inst:handleResponseInfo (msgInfo, onReceiveDone)
   ...
   if voucherCount >= 1 then
    ...
    if msgInfo.freeTimes ~= nil then
        ...
        if self.summonFreeQuota == -1 then self.summonFreeQuota = 0 end
    ...
    ---- 保底 24/6/5移除
    --local mID=ConfigManager.getPickUpCfg()[1].id
    --local HeroName=common:getLanguageString("@HeroName_"..mID)
    --if msgInfo.leftAwardTimes ~= nil then
    --    NodeHelper:setStringForTTFLabel(self.container, {
    --        summonPittyLeftDescTxt = common:getLanguageString("@Summon.SpecialpittyDesc", msgInfo.leftAwardTimes,HeroName)
    --    })
    --end
...
function Inst:onHandNode()
    ...
    local mID=self:getCfg().id
    rolePage:setMercenaryId(mID)
    --NgArchivePage_setToSkin(false, 1)
end

function Inst:getCfg()
    local PickUpCfg = ConfigManager.getPickUpCfg()
    local Cfg = nil
    for i = 1, #PickUpCfg do
        if PickUpCfg[i].actId == self.subPageCfg.activityID then
            Cfg = PickUpCfg[i]
            break
        end
    end
