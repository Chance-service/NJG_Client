...
--[[ 頁面名稱 ]]
Inst.pageName = "Summon.SummonGuideResultPage"

--[[ UI檔案 ]]
Inst.ccbiFile = "SummonGuideResult.ccbi"
...
function Inst:onEnter (container)
    ...
    strMap["resummonTxt"] = common:getLanguageString("@Summon.Again")
    ...
    GuideManager.PageContainerRef["SummonGuideResultPage"] = container
    GuideManager.PageInstRef["SummonGuideResultPage"] = slf
    if GuideManager.isInGuide and GuideManager.currGuide[GuideManager.currGuideType] == 70004 then
...
--[[ 播放所有 ]]
function Inst:playAll (isGuideJump)
    local slf = self
    Async:waterfall({
        function (nxt)
            slf:preloadRewards()
        ...
        function (nxt)
            if SummonSubPage_Base.JumpAnim or isGuideJump then
               nxt()
            else
