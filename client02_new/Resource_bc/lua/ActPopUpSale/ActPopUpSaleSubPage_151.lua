...
function ActPopUpSaleSubPage_151:initData()
    _buyData = ActPopUpSaleSubPage_151_getIsShowMainSceneIcon(GiftId)
end
function ActPopUpSaleSubPage_151_setGiftId(_id)
    GiftId = _id
end
-- 
function ActPopUpSaleSubPage_151:initUi(container)
	...
	itemInfo.price=ActPopUpSalePage_getPrice(itemInfo.id + 400)
...
function ActPopUpSaleSubPage_151_setServerData(msg)
    ...
    for id,data in pairs(_serverData) do
	    if msg.limitDate > 0 and type(id) == "number" then
	    	...
	    	_buyData=ActPopUpSaleSubPage_151_getIsShowMainSceneIcon(id)
	    	PopUpPage:setTime(151, _serverData.limitDate,_serverData[id].isGot,GiftId)
	    end
    end
end

function ActPopUpSaleSubPage_151_getIsShowMainSceneIcon(GiftId)
    if not GiftId then  
