...
-------------------------------------------------------
--[[
對隨機2/2/2/2/3/3/3名(params1)敵方造成100/120/140/160/180/200/220%(params2)傷害，
並賦予自身"荊棘I/荊棘I/荊棘II/荊棘II/荊棘III/荊棘III/荊棘III"(params3)12秒(params4)
]]--
-------------------------------------------------------
function Skill_50031:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_50031:calSkillTarget(chaNode, skillId)
    return nil
end
function Skill_50031:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
...
function Skill_50031:getSkillTarget(chaNode, skillId)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
    local skillLevel = skillId % 10
    local params = common:split(skillCfg.values, ",")
