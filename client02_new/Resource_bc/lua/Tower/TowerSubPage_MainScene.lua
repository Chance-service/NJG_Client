...
function TowerBase:onBack(container)
    self:scrollToLevel(container.mScrollview, #PageInfo.takeId, PageInfo.totalLevels, nil)
end
...
function TowerBase:setRewardPopCCB(container)
    ...
    if #PageInfo.takeId > 1 then
        local options = { height = 146, durationPerLevel = 0 }
        local floor = math.max(#PageInfo.takeId - 1, 1)
...
function TowerBase:onEnter(container)
    ...
    PageInfo = TowerDataBase:getData(194)
    ...
    PageInfo.isPassLevel = (TowerPass ~= #PageInfo.takeId)
    
    self:buildScrollview(container)
    self:setPage(container)
    NodeHelper:setNodesVisible(container,{mBuffNode = true,mTimeNode = true})
    NodeHelper:setStringForLabel(container,{mTowerTitle = common:getLanguageString("@SeasonTowerTitle")})
    NodeHelper:setSpriteImage(container,{mBg = "BG/Tower/Tower_bg.png"})
end
...
function TowerBase:initScrollToCurrentLevel(scrollView)
    if PageInfo.isFirstEnter then
        ...
        self:scrollToLevel(scrollView, #PageInfo.takeId, PageInfo.totalLevels)
    elseif PageInfo.isPassLevel then
        local options = { durationPerLevel = 0 }
        local actionArray = CCArray:create()
        actionArray:addObject(CCCallFunc:create(function()
            self:scrollToLevel(scrollView, #PageInfo.takeId - 1, PageInfo.totalLevels, nil, options)
        end))
        actionArray:addObject(CCDelayTime:create(0.5))
        actionArray:addObject(CCCallFunc:create(function()
            options.durationPerLevel = 0.2
            self:scrollToLevel(scrollView, #PageInfo.takeId, PageInfo.totalLevels, nil, options)
            local passKey = "TOWERPASS_" .. UserInfo.playerInfo.playerId
            CCUserDefault:sharedUserDefault():setIntegerForKey(passKey, #PageInfo.takeId)
        end))
...
function TowerBase:passLevel(scrollView)
    if #PageInfo.takeId < PageInfo.totalLevels then
        -- 播放當前層通關動畫（如有需要）
        self:scrollToLevel(scrollView, #PageInfo.takeId, PageInfo.totalLevels, function()
            -- 此處可加入解鎖下一層動畫
        end)
    else
...
function TowerContent:onRefreshContent(content)
   ...
   local isFirst = (self.index == 1)
    ...
    if data.id <= #PageInfo.takeId then
    ...
    local status = TowerBase:isLevelVisible(selfContainer.mScrollview, #PageInfo.takeId, PageInfo.totalLevels)
...
function RewardContent:onRefreshContent(content)
    ...
    if (self.data.id <= #PageInfo.takeId ) then
