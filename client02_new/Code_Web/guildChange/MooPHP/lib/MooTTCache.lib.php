...
class MooTTCache implements MooTTCacheInterface {
	...
	public static function get($key, $realExpired = 0, $serverId = false) {
		...
		// 把数据还原为数组，TT不会自动还原为数组
		$ttValue = $ttValue ? unserialize($ttValue) : null;
	...
	private static function _getInstance() {
		if (is_object($this) && $this instanceof MooTTCache) {
			return $this;
		}
		static $cache = null;
		if (!$cache) {
			$cache = new MooTTCache();
		}
		return $cache;
	}
}
