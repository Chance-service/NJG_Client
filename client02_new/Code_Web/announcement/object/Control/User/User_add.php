...
class Control_User_add {
	...
	function add() {
		...
		if($loginName) {
			...
			$area_0		= MooForm::request('area_0');			
			$area_1		= MooForm::request('area_1');		
			if(!$area_0) {
				$area_0 = 0;
			}	
			if(!$area_1) {
			...
			$areaPermit = $area_0 . "|" . $area_1;
			$gameIds = substr($gameIds, 0, -1);
			
			$add = MooObj::get('User')->add($loginName, $password, $name, $gameIds, $role, $areaPermit);
			$res = array();
			if (!$add) {
				$res['msg'] = "添加失败";
				$res['code'] = 0;
			...
			$dao = MooDao::get('Game');
			$gameList = $dao->getAll('select * from @TABLE');
