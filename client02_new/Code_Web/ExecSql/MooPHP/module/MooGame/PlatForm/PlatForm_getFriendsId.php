...
class Mod_MooGame_PlatForm_getFriendsId {
	...
	function getFriendsId($pageInfo = array()) {
	...
	function runOS($hashData, $pageInfo) {
		if ($pageInfo) {
			return MooMod::get('MooGame_PlatForm')->getFriendsId($hashData, $pageInfo);
		} else {
			return MooMod::get('MooGame_PlatForm')->getFriendsId($hashData);
		}
	}
	...
	function runF8($hashData, $pageInfo) {
		...
		if ($pageInfo) {
			$friendsId = MooMod::get('MooGame_PlatForm_'.$platForm)->getFriendsId($hashData, $pageInfo);
		} else {
			$friendsId = MooMod::get('MooGame_PlatForm_'.$platForm)->getFriendsId($hashData);
		}
		return $friendsId;
	}
