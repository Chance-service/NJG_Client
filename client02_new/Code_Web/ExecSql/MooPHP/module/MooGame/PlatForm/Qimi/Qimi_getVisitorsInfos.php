<?php
class Mod_MooGame_PlatForm_Qimi_getVisitorsInfos {
	...
	public function getVisitorsInfos($hashData, $fields, $uids = '') {
		...
		foreach($data as $key => $userInfo) {
			...
			$tempArr['uName'] = $userInfo['name'];
			$tempArr['uPic'] = $userInfo['avatar'];

			//note 保持和getVisitorInfo接口返回的性别信息一致性
			if(isset($userInfo['sex'])) {
				$tempArr['uSex'] = $userInfo['sex'] == 1 ? 'MALE' : 'FEMALE';
			}
