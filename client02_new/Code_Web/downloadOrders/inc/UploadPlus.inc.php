...
final class UploadPlus {
	 ...
	 * @return mix
	 */
	static public function put($FILE, $fileUrl='') {
		$upObj = self::_getInstance();

		$name		= $FILE["name"]; // 文件名
		$tmpName	= $FILE["tmp_name"]; // 文件临时路径
		$error		= $FILE['error']; // 错误
		$hz			= strtolower(substr(strrchr($name, '.'), 1)); // 文件后缀
	...
	private function _getInstance() {
		if (is_object($this) && $this instanceof UploadPlus) {
			return $this;
		}
		static $upObj = null;
		if (!$upObj) {
			$upObj = new UploadPlus();
		}
		return $upObj;
	}
}