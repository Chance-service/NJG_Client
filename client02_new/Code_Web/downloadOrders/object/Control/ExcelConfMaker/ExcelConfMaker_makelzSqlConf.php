 ...
 */
class Control_ExcelConfMaker_makelzSqlConf {
	var $theConf 	= array();
	var $fileName 	= 'lz_sql_hosts.conf.xls';
	var $tpl 		= ''; // 生成配置文件的模板
	var $confPHPTpl = '<?php
$CONF = ';
	
	public function makelzSqlConf() {	
	...
	public function setConf() {
		...
		foreach ($rsTestServer as $key => $val) {
			...
			$result[$type][$val['serverId']]['pwd'] 	= trim($val['cdb_password']);
		...
		MooFile::write(ROOT_PATH . '/conf/lz_execsqldbconfig.conf.php', $confTpl);
