...
--
-- 表的结构 `adventurelimittimecheckpoints`
--

CREATE TABLE IF NOT EXISTS `adventurelimittimecheckpoints` (
  `player_id` int(11) unsigned NOT NULL COMMENT '玩家id',
  `player_name` varchar(64) NOT NULL COMMENT '玩家名称',
  `platform_uid` varchar(64) NOT NULL COMMENT '平台用户id',
  `open_time` int(11) unsigned NOT NULL COMMENT '玩家获得开始时间',
  `reward_info` varchar(200) NOT NULL COMMENT '奖励领取信息，格式{1001:1,1002:0},副本id:是否领取,1为领取 0为未领取',
  `email_status` tinyint(1) unsigned NOT NULL COMMENT '发送邮件状态',
  PRIMARY KEY (`player_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='限时抢购活动';
...
CREATE TABLE IF NOT EXISTS `alliance` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `name` varchar(100) DEFAULT NULL COMMENT '帮派名称',
  `level` int(11) DEFAULT NULL COMMENT '帮派等级',
  `totalContribution` int(11) DEFAULT NULL COMMENT '帮派总贡献',
  `currentContribution` int(11) DEFAULT NULL COMMENT '当前贡献',
  `playerId` int(11) DEFAULT NULL COMMENT '帮主id',
  `notice` varchar(255) DEFAULT NULL COMMENT '帮派公告',
  `password` varchar(50) DEFAULT NULL COMMENT '军团密码',
  `createTime` bigint(20) DEFAULT NULL COMMENT '创建时间',
  `useCanBaiCount` int(11) DEFAULT NULL COMMENT '关公殿参拜次数',
  `declaration` varchar(255) DEFAULT NULL COMMENT '军团宣言',
  `isDelete` int(11) DEFAULT '0' COMMENT '是否删除',
  `updateCanBaiTime` bigint(20) NOT NULL DEFAULT '0' COMMENT '参拜时间',
  PRIMARY KEY (`id`)
...
CREATE TABLE IF NOT EXISTS `alliance_apply` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `allianceId` int(11) NOT NULL DEFAULT '0' COMMENT '军团id',
  `playerId` int(11) DEFAULT NULL COMMENT '玩家id',
  `createTime` bigint(20) DEFAULT NULL COMMENT '申请时间',
  `status` int(11) DEFAULT '0' COMMENT '申请状态：0未审核，1通过，2拒绝',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `alliance_building` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `allianceId` int(11) NOT NULL COMMENT '帮派id',
  `type` int(11) NOT NULL DEFAULT '0' COMMENT '建筑类型',
  `level` int(11) DEFAULT NULL COMMENT '建筑等级',
  PRIMARY KEY (`id`),
  KEY `allianceId` (`allianceId`)
...
CREATE TABLE IF NOT EXISTS `alliance_dynamic` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `allianceId` int(11) DEFAULT NULL COMMENT '帮派id',
  `params` varchar(255) DEFAULT '' COMMENT '动态参数，格式：A_B_C',
  `createTime` bigint(20) DEFAULT NULL COMMENT '创建时间',
  `playerId` int(11) DEFAULT NULL COMMENT '玩家id',
  `type` int(11) DEFAULT NULL COMMENT '动态类型',
  PRIMARY KEY (`id`),
  KEY `allianceId` (`allianceId`)
...
CREATE TABLE IF NOT EXISTS `battle` (
  ...
  `dragonSoulArrayStr` varchar(64) COLLATE utf8_bin NOT NULL,
  PRIMARY KEY (`id`),
  KEY `playerId_index` (`playerId`) USING BTREE,
...
CREATE TABLE IF NOT EXISTS `collect_card` (
  ...
  `collect_content` text NOT NULL COMMENT '收集内容',
  `collect_status` varchar(200) NOT NULL COMMENT '收集状态',
  `collect_time` text NOT NULL COMMENT '收集时间',
  `player_puid` varchar(64) NOT NULL COMMENT 'puid',
  `collect_opentime` int(11) NOT NULL COMMENT '活动开启时间',
  PRIMARY KEY (`playerid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='收集神卡表';
...
CREATE TABLE IF NOT EXISTS `player` (
  ...
  `dragonSoulFragment` int(11) NOT NULL,
  `monthlyCardRechargeNum` int(11) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`),
  KEY `puid_index` (`puid`) USING BTREE,
...
CREATE TABLE IF NOT EXISTS `player_alliance` (
  `playerId` int(11) NOT NULL AUTO_INCREMENT COMMENT '玩家id',
  `allianceId` int(11) DEFAULT NULL COMMENT '所属帮派id',
  `totalContribution` int(11) DEFAULT NULL COMMENT '总贡献度',
  `currentContribution` int(11) DEFAULT NULL COMMENT '当前贡献度',
  `lastContributionTime` bigint(20) DEFAULT NULL COMMENT '上次捐献时间',
  `lastGuanGongTime` bigint(20) DEFAULT NULL COMMENT '上次参拜时间',
  `status` int(11) DEFAULT NULL COMMENT '是否加入军团中未加入：0；已加入：1',
  `postion` int(11) DEFAULT NULL COMMENT '军团职位',
  `exitAllianceTime` bigint(20) DEFAULT NULL COMMENT '退出军团时间',
  `jointTime` bigint(20) DEFAULT '0' COMMENT '加入军团时间',
  PRIMARY KEY (`playerId`),
  KEY `allianceId` (`allianceId`)
...
CREATE TABLE IF NOT EXISTS `player_distrial` (
  ...
  `buyLightSoulCount` int(11) DEFAULT '0' COMMENT '购买魂灯次数',
  `alreadyNpc` text COMMENT '已经打过的npc',
  `isAddAtrr` int(11) NOT NULL DEFAULT '0' COMMENT '是否已经加属性',
  PRIMARY KEY (`playerId`)
...
CREATE TABLE IF NOT EXISTS `rewards` (
  ...
  `rewardInfo` varchar(128) COLLATE utf8_bin DEFAULT NULL,
...
--
-- 表的结构 `new_server_sendaward`
--

CREATE TABLE IF NOT EXISTS `new_server_sendaward` (
  `nss_type_id` int(11) NOT NULL COMMENT '活动类型Id',
  `nss_status` tinyint(2) NOT NULL COMMENT '发邮件状态',
  `nss_time` int(11) NOT NULL COMMENT '发奖励时间',
  PRIMARY KEY (`nss_type_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='新服活动全服发邮件表';
