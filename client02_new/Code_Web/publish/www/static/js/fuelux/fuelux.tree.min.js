(function(a, c) {
    var b = function(e, d) {
        ...
        this.$element.on("click", ".tree-item", a.proxy(function(f) {
			var currentTarget = a(f.currentTarget);
			var currentTargetData = currentTarget.data();
			if(currentTargetData.tag == 'all') {
				var pd = currentTarget.parent();
				var pe = a(pd[0]);
				var peItem = pe.find(".tree-item");
				if(currentTarget.hasClass("tree-selected")) {
					for(var i = 0; i < peItem.length; i++) {
						var tmpd = a(peItem[i]);
						if (!tmpd.hasClass("tree-selected")) {
							this.selectItem(peItem[i]);
						}
					}
				} else {
					for(var i = 0; i < peItem.length; i++) {
