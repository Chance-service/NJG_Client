...
@synthesize delegateEncode = _delegateEncode;
...
- (NSData*)DecodeSpeexToWAVE:(NSData *)data
{
    if (data == nil){
        ...
        [self.delegateDecode decodeFailed:self];
    ...
    short pcmFrame[FRAME_SIZE];
    spx_int16_t output[FRAME_SIZE];//zhenji yongde hanshu speex_decode,canshu shi spx_int16_tde
    ...
    [self.delegateDecode decodeSucceed:self WithData:outData];
...
- (NSData*)EncodeWAVEToSpeex:(NSData *)data withChannels:(int)nChannels andBitsPerSample:(int)nBitsPerSample
{
    if (data == nil){
        ...
        [self.delegateEncode encodeFailed:self];
    ...
    [self.delegateEncode encodeSucceed:self WithData:speexData];
...
- (void)dealloc
{
    [_delegateEncode release];
    _delegateEncode = nil;
    
    [_delegateDecode release];
    _delegateDecode = nil;
