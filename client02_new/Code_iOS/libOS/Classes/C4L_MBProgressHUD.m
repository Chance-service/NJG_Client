...
@implementation C4L_MBProgressHUD
...
- (void)updateProgress {
    [(C4L_MBRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeDeterminate) {
        self.indicator = [[[C4L_MBRoundProgressView alloc] init] autorelease];
    }
...
+ (C4L_MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	C4L_MBProgressHUD *hud = [[C4L_MBProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[C4L_MBProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		C4L_MBProgressHUD *HUD = (C4L_MBProgressHUD *)viewToRemove;
...
@implementation C4L_MBRoundProgressView
