...
//#ifdef LIBOS
////for 这个是晓东的证书 wangxiaodong@com4loves.com
//static const char kKeyChainUDIDAccessGroup[] = "Q498LWL48Q.com.loves.keychainshare";
//#endif
...
//static const char kKeyChainUDIDAccessGroup[] = "GY4QY745E5.com.loves.keychainshare";
//#endif
...
//static NSString* keyChainUDIDAccessGroup = @"Q498LWL48Q.com.loves.keychainshare";
static NSString* keyChainUDIDAccessGroup = @"2WLZ3CD2G8.jp.co.school.battle.keychainshare";
...
{
    if(keyChainUDIDAccessGroup)
    {
      //  [keyChainUDIDAccessGroup release];
        keyChainUDIDAccessGroup = [keyChainUDIDAccessGroup retain];
    }
}
...
+ (NSString *) IDFAWithKeychain
{
    ...
    if (!idfa||[idfa length]<=0) {
        YAKeychainWrapper *keychain=[[YAKeychainWrapper alloc] initWithIdentifier:@"youaiKeychain" accessGroup:keyChainUDIDAccessGroup];
        [idfa release];
...
+ (NSString*)UDID
{
//    static NSString *strTime;
//    if(strTime == nil)
//    {
//        NSDate *now = [NSDate date];
//        NSDateFormatter *formatter = [[NSDateFormatter  alloc ]  init ];
//        [formatter setDateFormat:@"yyyy-MM-dd-hh-mm-ss"];
//        strTime = [formatter stringFromDate:now];
////        NSLog(@"now date is: %@", todayTime);
//    }
//    return strTime;
    if (!udid) {
        [udid release];
...
+ (NSString*)getUDIDFromKeyChain
{
    ...
    if (keyChainUDIDAccessGroup != nil)
    {
...
#else
        [dictForQuery setObject:keyChainUDIDAccessGroup forKey:(id)kSecAttrAccessGroup];
#endif
    }
...
+ (BOOL)settUDIDToKeyChain:(NSString*)udid
{
    ...
    if (keyChainUDIDAccessGroup != nil)
    {
        ...
        // simulator will return -25243 (errSecNoAccessForItem).
#else
        [dictForAdd setObject:keyChainUDIDAccessGroup forKey:(id)kSecAttrAccessGroup];
#endif
    }
