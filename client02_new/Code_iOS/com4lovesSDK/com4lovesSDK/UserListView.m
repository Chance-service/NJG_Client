...
- (void)viewDidLoad
{
    ...
    [lableTitle setText:[comHuTuoSDK getLang:@"userlist_title"]];
    [btnCancle setTitle:[comHuTuoSDK getLang:@"userlist_cancle"] forState:UIControlStateNormal];
    [btnCancle setTitle:[comHuTuoSDK getLang:@"userlist_cancle"] forState:UIControlStateSelected];
    [btnCancle setTitle:[comHuTuoSDK getLang:@"userlist_cancle"] forState:UIControlStateHighlighted];
...
-(void) refreshResp
{
    ...
    for (HuTuoUser *user in values) {
        if (user.userType!=2) {
            [listData addObject:user.huTuoId];
        }
    }
...
//    [cell.imageView setImage:img];
    ...
    HuTuoUser *user = [[[ServerLogic sharedInstance] getUserList] objectForKey:cellname];
    cellname = user.name;
    NSString* defaultname = [[ServerLogic sharedInstance] getLatestUser];
    if ([cellname isEqualToString:defaultname]) {
            [cell.detailTextLabel setText:[comHuTuoSDK getLang:@"defualt"]];
...
-(void)buttonclick:(id)sender
{
    ...
    HuTuoUser *user = [[[ServerLogic sharedInstance] getUserList] objectForKey:theKey];
...
(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        {
            [[comHuTuoSDK sharedInstance] clearLoginInfo];
        }
