...
@interface libRyukNewObj ()

@property (nonatomic,retain)AdverView *adverView;

@end

@implementation libRyukNewObj
#pragma mark--
...
-(void) registerNotification{
    ...
    //监听恢复购买
    //[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(restoreSuccessFul:)  name:KInAppPurchaseManagerRestoreTransactionSucNotifaction object:nil];
...
-(void) unregisterNotification{
    ...
    //[[NSNotificationCenter defaultCenter] removeObserver:self name:KInAppPurchaseManagerRestoreTransactionSucNotifaction object:nil];
...
for( int i = 0; i <= retryTimes; ++i ){
    ...
    if(data){
        ...
        if([array count] >= 6){
           ...
           // NSString* isDataTransfer = array[6];//是否移行过 1 进行过移行 0 未移行过
             NSString* md5Code = [[[[SDKUtility sharedInstance]md5HexDigest:devicesId] uppercaseString] substringWithRange:NSMakeRange(8, 12)];
            if ([code isEqualToString:md5Code]) {
                libPlatformManager::getPlatform()->sendMessageG2P("P2G_DATATRANSFER_SHOW",[@"0" UTF8String]);
            }else
            {
                 libPlatformManager::getPlatform()->sendMessageG2P("P2G_DATATRANSFER_SHOW",[@"1" UTF8String]);
            }
            self.code = code;
            ...
            if([ok isEqualToString:@"1"]){//成功
                ...
                //[[NSNotificationCenter defaultCenter] postNotificationName:GNETOP_LOGIN_NOTIFICATION object:nil userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"12345678999" ,@"result",nil]];
                break;
            }
            else{
...
//
//        [[NSNotificationCenter defaultCenter] postNotificationName:GNETOP_LOGIN_NOTIFICATION object:nil userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"6676" ,@"result",nil]];
...
- (void)phchaseSuccessful:(NSNotification *)notification{
    ...
    libRyukNew* plat = dynamic_cast<libRyukNew * >(libPlatformManager::getPlatform());
    if (plat) {
        ...
        [[ShareSDKToDifPlatform shareSDKPlatform] sendTrackEventXX:(double)info.productPrice currency:@"JPY" differentToken:RYUKAdjustAppToken transactionId:trans.transactionIdentifier];
...
-(void )restoreSuccessFul:(NSNotification *) notifaction
{
    NSLog(@"restore successFull %s",__FUNCTION__);
    NSDictionary *userInfo = notifaction.userInfo;
    NSString *transId = (NSString *)[userInfo objectForKey:@"restoreTransaction"];
    NSString *productId = (NSString *)[userInfo objectForKey:@"restoreProductId"];
    libRyukNew *plat = dynamic_cast<libRyukNew *>(libPlatformManager::getPlatform());
    if (plat) {
        BUYINFO info = plat->getBuyInfo();
        std::string _transId = [transId UTF8String];
        std::string _productid = [productId UTF8String];
        libPlatformManager::getPlatform()->sendMessageP2G("G2P_MONTHCARDRESTORE", _productid + "_" +  _transId);
    }
}
...
- (void) changePwd:(NSString*) pwd{
    NSString *changePassword = [comHuTuoSDK getPropertyFromIniFile:@"Account" andAttr:@"dataTransferChangePwdUrl"];
   
    //libOS::getInstance()->setWaiting(true);
    //dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),^{
        NSString *str = [NSString stringWithFormat:changePassword,pwd,self.code];
    
        NSString* data = [[SDKUtility sharedInstance]httpRequest:str postData:nil];
        NSLog(@"changeCode receive data = %@",data);
        NSMutableDictionary *m_dic = [NSMutableDictionary dictionary];
        if(data){
            NSArray *array = [data componentsSeparatedByString:@"|"];
            if([array count] >= 2){
                NSString *ok = array[0];//成功标志 1 成功 0 失败
                NSString *state = array[1];//失败原因 1参数缺失或者密码长度问题 2 没有找到该accode对应的账号记录
                [m_dic setValue:ok forKey:@"result"];
                [m_dic setValue:state forKey:@"code"];
                if([ok isEqualToString:@"1"]){
                    [m_dic setValue:pwd forKey:@"pwd"];
                }
            }
            else{
                [m_dic setValue:@"0" forKey:@"result"];
                [m_dic setValue:@"2" forKey:@"code"];
            }
        }
        NSLog(@"--------------sendMessageP2G:----------P2G_CHANGE_PWD----%@",m_dic);
        NSError *parseError = nil;
        NSData  *jsonData = [NSJSONSerialization dataWithJSONObject:m_dic options:NSJSONWritingPrettyPrinted error:&parseError];
        NSString *productJson = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
        libPlatformManager::getPlatform()->sendMessageP2G("P2G_CHANGE_PWD",[productJson UTF8String]);
        libOS::getInstance()->setWaiting(false);
    //});
}
//请求数据移行
- (void) changeCode:(NSString*)code pWd:(NSString*)pwd{
    
    //dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),^{
    //libOS::getInstance()->setWaiting(true);
    NSString *changeCode = [comHuTuoSDK getPropertyFromIniFile:@"Account" andAttr:@"dataTransferUrl"];
    bool isChangeSuccess = false;
    NSString *str = [NSString stringWithFormat:changeCode,pwd,code,self.deviceid];
    NSString* data = [[SDKUtility sharedInstance]httpRequest:str postData:nil];
    NSLog(@"changeCode receive data = %@",data);
    NSMutableDictionary *m_dic = [NSMutableDictionary dictionary];
    if(data){
        ...
        if([array count] >= 2){
            NSString *ok = array[0];//成功标志 1 成功 0 失败
            NSString *state = array[1];//失败原因 1参数缺失或者密码长度问题 2 没有找到该accode对应的账号记录 3 密码错误 4 该设备已经进行过了移行
    ...
    NSLog(@"--------------sendMessageP2G:----------P2G_DATA_TRANSFER----%@",m_dic);
    NSError *parseError = nil;
    NSData  *jsonData = [NSJSONSerialization dataWithJSONObject:m_dic options:NSJSONWritingPrettyPrinted error:&parseError];
    NSString *productJson = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
    libPlatformManager::getPlatform()->sendMessageP2G("P2G_DATA_TRANSFER",[productJson UTF8String]);
    libOS::getInstance()->setWaiting(false);
    if(isChangeSuccess){
        [self RequestAccountServer:self.deviceid gcID:self.gcid];
    }
